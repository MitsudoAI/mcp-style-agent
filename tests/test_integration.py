...
def simulate_original_bug():
    ...
    print(
        f"✅ Step 1: Decomposition completed - found {len(decomposition_result['sub_questions'])} sub-questions:"
    )
...
def simulate_fixed_evidence_collection(decomposition_result):
    ...
    for i, sub_question in enumerate(decomposition_result["sub_questions"]):
        ...
        evidence_result = {
            ...
            "sources_found": len(sub_question["search_keywords"])
            * 2,  # Simulate finding sources
...
def simulate_broken_behavior():
    ...
    print(
        f"❌ Evidence collection completed for ONLY {len(evidence_results)} sub-question (SQ1)"
    )
...
def main():
    ...
    print(
        f"Coverage improvement: {len(fixed_results) - len(broken_results)} additional sub-questions"
    )
    ...
    if len(fixed_results) == expected_count:
        ...
        for result in fixed_results:
            print(
                f"   ✓ {result['question_id']}: Evidence ready for critical evaluation"
            )
