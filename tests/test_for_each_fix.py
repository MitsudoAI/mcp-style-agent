...
def test_for_each_reference_resolution():
    ...
    result = executor._resolve_for_each_reference("decomposer.sub_questions", step_outputs, context)
    assert len(result) == 2
    assert result[0]["id"] == "SQ1"
    assert result[1]["id"] == "SQ2"
    
    # Test invalid reference format
    result = executor._resolve_for_each_reference("invalid_format", step_outputs, context)
    assert result == []
    
    # Test missing step
    result = executor._resolve_for_each_reference("missing_step.sub_questions", step_outputs, context)
    assert result == []
    
    # Test missing property
    result = executor._resolve_for_each_reference("decomposer.missing_property", step_outputs, context)
    assert result == []
...
def test_for_each_error_handling():
    ...
    result = executor._execute_step_with_for_each(
        "test_flow", step, "test.items", {}, 
        {"test": {"items": iteration_data}}, continue_on_error=True
    )
