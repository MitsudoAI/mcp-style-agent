...
from src.mcps.deep_thinking.config.user_config_manager import (
    ConfigPriority,
    UserConfigManager,
)
...
class TestUserConfigManager(unittest.TestCase):
    ...
    def test_profile_inheritance(self):
        ...
        async def run_test():
            ...
            effective_config = self.manager.get_effective_config(
                "system", profile="derived_profile"
            )
            ...
            # Check agents config
            agents_config = self.manager.get_effective_config(
                "agents", profile="derived_profile"
            )
    ...
    def test_config_export_import(self):
        ...
        async def run_test():
            ...
            self.manager.export_config(
                export_file, include_profiles=True, format="yaml"
            )
    ...
    def test_config_validation(self):
        ...
        async def run_test():
            ...
            validation_result = self.manager.validate_user_config(
                "system", profile="valid_profile"
            )
    ...
    def test_deep_merge(self):
        ...
        self.assertEqual(
            merged["level1"]["level2"]["setting2"], "new_value2"
        )  # Overridden
