...
class TestFlowInterruptionRecovery:
    ...
    def test_detect_flow_interruption_no_interruption(
        self, session_manager, sample_session
    ):
        ...
        sample_session.step_results["collect_evidence"] = {
    ...
    def test_detect_flow_interruption_session_timeout(
        self, session_manager, sample_session
    ):
    ...
    def test_detect_flow_interruption_incomplete_step(
        self, session_manager, sample_session
    ):
    ...
    def test_detect_flow_interruption_quality_gate_failure(
        self, session_manager, sample_session
    ):
    ...
    def test_detect_flow_interruption_missing_prerequisites(
        self, session_manager, sample_session
    ):
        ...
        sample_session.step_results = {
            ...
            }  # Missing collect_evidence prerequisite
        }
        sample_session.updated_at = datetime.now()
        ...
        assert ("multi_perspective_debate", "collect_evidence") in result[
            "missing_prerequisites"
        ]
        assert "execute_missing_steps" in result["recovery_options"]

    def test_detect_flow_interruption_unexpected_step_sequence(
        self, session_manager, sample_session
    ):
        ...
        sample_session.current_step = (
            "critical_evaluation"  # Should be collect_evidence
        )
        sample_session.updated_at = datetime.now()
    ...
    def test_rollback_to_step_success(self, session_manager, sample_session, mock_db):
        ...
        result = session_manager.rollback_to_step(
            sample_session.session_id, "collect_evidence"
        )
    ...
    def test_rollback_to_step_target_not_found(self, session_manager, sample_session):
        ...
        result = session_manager.rollback_to_step(
            sample_session.session_id, "nonexistent_step"
        )

        assert result is False

    def test_create_recovery_checkpoint_success(
        self, session_manager, sample_session, mock_db
    ):
        ...
        checkpoint_id = session_manager.create_recovery_checkpoint(
    ...
    def test_restore_from_checkpoint_success(
        ...
        result = session_manager.restore_from_checkpoint(
            sample_session.session_id, checkpoint_id
        )
        ...
        assert (
            sample_session.context["restored_from_checkpoint"]["checkpoint_id"]
            == checkpoint_id
        )
    ...
    def test_restore_from_checkpoint_session_not_found(self, session_manager):
        ...
        result = session_manager.restore_from_checkpoint(
            "nonexistent-session", "some-checkpoint"
        )

        assert result is False

    def test_restore_from_checkpoint_checkpoint_not_found(
        self, session_manager, sample_session
    ):
        ...
        result = session_manager.restore_from_checkpoint(
            sample_session.session_id, "nonexistent-checkpoint"
        )
    ...
    def test_get_expected_steps_for_flow(self, session_manager):
        ...
        comprehensive_steps = session_manager._get_expected_steps_for_flow(
    ...
    def test_detect_flow_interruption_exception_handling(self, session_manager):
        ...
        with patch.object(
            session_manager, "get_session", side_effect=Exception("Database error")
        ):
    ...
    def test_rollback_database_failure_continues(
        self, session_manager, sample_session, mock_db
    ):
        ...
        result = session_manager.rollback_to_step(
            sample_session.session_id, "decompose_problem"
        )
    ...
    def test_checkpoint_database_failure_continues(
        self, session_manager, sample_session, mock_db
    ):
    ...
    def test_interruption_detection_logging(
        self, mock_logger, session_manager, sample_session
    ):
