...
class TemplateOutputQualityTester:
    ...
    def test_output_quality(self, template_content: str, template_name: str) -> Dict[str, Any]:
        ...
        structure_result = self._test_structure_completeness(template_content, template_name)
        ...
        results['overall_score'] = (
            ...
            results['instruction_score'] * self.quality_metrics['instruction_following']
        )
        
        return results
    
    def _test_structure_completeness(self, content: str, template_name: str) -> Dict[str, Any]:
        ...
        structure_elements = [has_title, has_context, has_task_description, has_output_format]
        ...
        if 'decomposition' in template_name:
            ...
            if has_decomp_strategy:
                result['strengths'].append("Decomposition template has strategy guidance")
            else:
    ...
    def _test_content_relevance(self, content: str, template_name: str) -> Dict[str, Any]:
        ...
        specific_instructions = len(re.findall(r'必须|应该|确保|至少|不少于|\d+个', content))
    ...
    def _test_format_compliance(self, content: str) -> Dict[str, Any]:
        ...
        for i, json_block in enumerate(json_blocks):
            try:
                ...
                cleaned_json = re.sub(r'"[^"]*\{[^}]+\}[^"]*"', '"template_placeholder"', cleaned_json)
                ...
                if '{' in json_block and '}' in json_block:
                    result['strengths'].append(f"JSON block {i+1} appears to be a valid template")
                else:
        ...
        if has_numbered_lists or has_bullet_lists:
            ...
            result['issues'].append("Template could benefit from better list formatting")
            format_score -= 0.1
    ...
    def _test_instruction_following(self, content: str) -> Dict[str, Any]:
        ...
        action_verbs = re.findall(r'(请|需要|要求|执行|分析|评估|生成|创建|识别|检测)', content)
        ...
        specific_reqs = re.findall(r'(必须|应该|确保|至少|不少于|不超过|\d+个|\d+分)', content)
...
class TemplateInstructionClarityTester:
    ...
    def test_instruction_clarity(self, template_content: str) -> Dict[str, Any]:
        ...
        results['overall_clarity_score'] = (
            ...
            results['structure_clarity_score'] * self.clarity_factors['structure_clarity'] +
            results['requirement_clarity_score'] * self.clarity_factors['requirement_clarity']
        )
    ...
    def _test_structure_clarity(self, content: str) -> Dict[str, Any]:
        ...
        if has_sections:
            ...
            result['issues'].append("Template could benefit from better section organization")
    ...
    def _test_requirement_clarity(self, content: str) -> Dict[str, Any]:
        ...
        if has_format_spec:
            ...
            result['issues'].append("Template should specify output format more clearly")
...
class TemplateEffectTester:
    ...
    def test_template_effectiveness(self, template_content: str, template_name: str) -> TemplateEffectTestResult:
        """Perform comprehensive template effectiveness testing"""
        
        # Test output quality
        quality_results = self.output_quality_tester.test_output_quality(template_content, template_name)
        
        # Test instruction clarity
        clarity_results = self.instruction_clarity_tester.test_instruction_clarity(template_content)
    ...
    def _generate_quality_recommendations(self, quality_results: Dict[str, Any]) -> List[str]:
        ...
        if quality_results['structure_score'] < 0.7:
            recommendations.append("Improve template structure with clearer sections and organization")
        
        if quality_results['content_score'] < 0.7:
            recommendations.append("Enhance content relevance and reduce vague language")
        
        if quality_results['format_score'] < 0.7:
            recommendations.append("Fix formatting issues, especially JSON examples and heading structure")
        
        if quality_results['instruction_score'] < 0.7:
            recommendations.append("Strengthen instructions with more action verbs and specific requirements")
        
        return recommendations
    
    def _generate_clarity_recommendations(self, clarity_results: Dict[str, Any]) -> List[str]:
        """Generate recommendations based on clarity test results"""
        recommendations = []
        
        if clarity_results['language_clarity_score'] < 0.7:
            recommendations.append("Use more specific and direct language, avoid vague terms")
        
        if clarity_results['structure_clarity_score'] < 0.7:
            recommendations.append("Improve structural organization with better section flow")
    ...
    def test_all_templates(self, templates_dir: str) -> Dict[str, TemplateEffectTestResult]:
    ...
    def generate_effectiveness_report(self, results: Dict[str, TemplateEffectTestResult]) -> str:
        ...
        avg_effectiveness = sum(r.effectiveness_score for r in results.values()) / total_templates
        avg_quality = sum(r.output_quality_score for r in results.values()) / total_templates
        avg_clarity = sum(r.instruction_clarity_score for r in results.values()) / total_templates
        
        high_quality_templates = sum(1 for r in results.values() if r.effectiveness_score >= 0.8)
        medium_quality_templates = sum(1 for r in results.values() if 0.6 <= r.effectiveness_score < 0.8)
        low_quality_templates = sum(1 for r in results.values() if r.effectiveness_score < 0.6)
        ...
        sorted_results = sorted(results.items(), key=lambda x: x[1].effectiveness_score, reverse=True)
        
        for template_name, result in sorted_results:
            status_emoji = "🟢" if result.effectiveness_score >= 0.8 else "🟡" if result.effectiveness_score >= 0.6 else "🔴"
...
"""
        ...
        result = self.tester.test_output_quality(high_quality_template, "high_quality_test")
...
"""
        ...
        result = self.tester.test_output_quality(low_quality_template, "low_quality_test")
...
"""
        ...
        assert specific_result['language_clarity_score'] > vague_result['language_clarity_score']
...
"""
        ...
        result = self.tester.test_template_effectiveness(test_template, "comprehensive_test")
...
"""
        ...
        good_result = self.tester.test_template_effectiveness(good_template, "good_test")
        poor_result = self.tester.test_template_effectiveness(poor_template, "poor_test")
...
"""
        ...
        result = self.tester.test_template_effectiveness(template_with_issues, "issues_test")
        ...
        assert any(keyword in rec_text for keyword in ['structure', 'clarity', 'specific', 'format'])
...
class TestTemplateEffectIntegration:
    ...
    def test_existing_templates_effectiveness(self):
        ...
        for name, result in results.items():
            ...
            print(f"Template {name}: Effectiveness {result.effectiveness_score:.2f}, "
        ...
        complete_templates = {name: result for name, result in results.items() 
                            if result.test_results.get('validation_result', {}).get('metrics', {}).get('word_count', 0) > 20}
        
        if complete_templates:
            # At least 70% of complete templates should have decent effectiveness
            good_templates = sum(1 for r in complete_templates.values() if r.effectiveness_score >= 0.6)
            total_complete = len(complete_templates)
            
            assert good_templates / total_complete >= 0.7, \
    ...
    def test_template_manager_integration(self):
        ...
        for template_name in template_names[:3]:  # Test first 3 templates
            try:
                ...
                original_content = self.template_manager.cache.get(template_name, template_content)
                
                result = self.tester.test_template_effectiveness(original_content, template_name)
                ...
                pytest.fail(f"Template manager integration failed for {template_name}: {e}")
