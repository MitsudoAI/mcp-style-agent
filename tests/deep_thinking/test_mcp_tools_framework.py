...
try:
    ...
    class MockMCPTools:
        ...
        def start_thinking(self, input_data):
            return MockMCPToolOutput(
                ...
                metadata={
                    "flow_type": getattr(
                        input_data, "flow_type", "comprehensive_analysis"
                    )
                },
            )
    ...
    class AnalyzeStepInput:
        def __init__(
            self, session_id="", step_name="", step_result="", analysis_type="quality"
        ):
            self.session_id = session_id
...
""",
    ...
    def _test_start_thinking_tool(self, mcp_tools: MCPTools) -> Dict[str, Any]:
        ...
        test_cases = [
            ...
            },
            {
                ...
                "input": StartThinkingInput(
                    topic="简单问题测试", complexity="simple", focus=""
                ),
                "expected_step": "decompose_problem",
            },
            {
        ...
        for test_case in test_cases:
            try:
                ...
                if validation["valid"]:
                    results["passed"] += 1
                    print(
                        f"  ✅ {test_case['name']}: {response_time:.2f}ms, Quality: {quality_score:.1f}/10"
                    )
                else:
                    results["failed"] += 1
    ...
    def _test_next_step_tool(self, mcp_tools: MCPTools) -> Dict[str, Any]:
        ...
        start_input = StartThinkingInput(topic="测试问题", complexity="moderate")
        ...
        test_cases = [
            ...
            },
            {
                ...
                "input": NextStepInput(
                    ...
                    quality_feedback={"quality_score": 8.5, "feedback": "优秀的分析"},
                ),
            ...
            },
            {
                ...
                "input": NextStepInput(
                    session_id=session_id,
                    step_result="简单结果",
                    quality_feedback={"quality_score": 0.4, "feedback": "质量需要改进"},
                ),
        ...
        for test_case in test_cases:
            try:
                ...
                results["failed"] += 1
                results["test_details"].append(
                    {"test_name": test_case["name"], "passed": False, "error": str(e)}
                )
                print(f"  ❌ {test_case['name']}: Exception - {e}")
    ...
    def _test_analyze_step_tool(self, mcp_tools: MCPTools) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            try:
                ...
                quality_score = self.quality_assessor.assess_analyze_step_quality(
                    result
                )
    ...
    def _test_complete_thinking_tool(self, mcp_tools: MCPTools) -> Dict[str, Any]:
        ...
        test_cases = [
            {
                ...
                "input": CompleteThinkingInput(session_id=session_id),
            ...
            },
            {
                ...
                "input": CompleteThinkingInput(session_id="nonexistent-session"),
        ...
        for test_case in test_cases:
            try:
                ...
                quality_score = self.quality_assessor.assess_complete_thinking_quality(
                    result
                )
    ...
    def _validate_start_thinking_result(
        self, result: MCPToolOutput, test_case: Dict
    ) -> Dict[str, Any]:
        ...
        if tool_name_value != "start_thinking":
            issues.append(
                f"Wrong tool name: expected 'start_thinking', got {tool_name_value}"
            )
        ...
        if result.step != test_case["expected_step"]:
            issues.append(
                f"Wrong step: expected {test_case['expected_step']}, got {result.step}"
            )
    ...
    def _validate_next_step_result(
        self, result: MCPToolOutput, test_case: Dict
    ) -> Dict[str, Any]:
        ...
        if tool_name_value != "next_step":
            issues.append(
                f"Wrong tool name: expected 'next_step', got {tool_name_value}"
            )
        ...
        if "expected_quality_gate" in test_case:
            ...
            if quality_gate_passed != test_case["expected_quality_gate"]:
                issues.append(
                    f"Quality gate mismatch: expected {test_case['expected_quality_gate']}"
                )
    ...
    def _validate_analyze_step_result(
        self, result: MCPToolOutput, test_case: Dict
    ) -> Dict[str, Any]:
        ...
        if tool_name_value != "analyze_step":
            issues.append(
                f"Wrong tool name: expected 'analyze_step', got {tool_name_value}"
            )

        if not result.step.startswith("analyze_") and not result.step.startswith(
            "format_validation_"
        ):
            issues.append(
                f"Step should start with 'analyze_' or 'format_validation_': {result.step}"
            )
        ...
        if test_case.get("expected_format_failure"):
            if not result.step.startswith("format_validation_"):
                issues.append(
                    "Expected format validation failure but got regular analysis"
                )

        # Check analysis type handling
        if "expected_analysis" in test_case:
            if result.step != test_case[
                "expected_analysis"
            ] and not result.step.startswith("format_validation_"):
                issues.append(
                    f"Wrong analysis step: expected {test_case['expected_analysis']}"
                )
    ...
    def _validate_complete_thinking_result(
        self, result: MCPToolOutput, test_case: Dict
    ) -> Dict[str, Any]:
        ...
        if tool_name_value != "complete_thinking":
            issues.append(
                f"Wrong tool name: expected 'complete_thinking', got {tool_name_value}"
            )
    ...
    def _calculate_test_coverage(self, test_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for tool_name, tool_results in test_results.items():
            ...
            coverage_by_tool[tool_name] = {
                ...
                "coverage_score": (
                    tool_results["passed"] / test_cases if test_cases > 0 else 0
                ),
            }
        ...
        return {
            ...
            "overall_coverage_score": (
                sum(tool["coverage_score"] for tool in coverage_by_tool.values())
                / len(coverage_by_tool)
                if coverage_by_tool
                else 0
            ),
        }
    ...
    def _generate_performance_summary(
        ...
        for tool_name, tool_results in test_results.items():
            response_times = [
                ...
                for test in tool_results["test_details"]
...
class ToolQualityAssessor:
    ...
    def assess_start_thinking_quality(
        self, result: MCPToolOutput, input_data: StartThinkingInput
    ) -> float:
    ...
    def assess_analyze_step_quality(self, result: MCPToolOutput) -> float:
        ...
        # Check step naming
        if not (
            result.step.startswith("analyze_")
            or result.step.startswith("format_validation_")
        ):
            score -= 2.0
...
class PerformanceMonitor:
    ...
    def assess_performance(
        self, tool_name: str, response_time_ms: float
    ) -> Dict[str, Any]:
