...
from src.mcps.deep_thinking.templates.template_manager import (
    TemplateManager,
    ConfigurationError,
)
...
class TestTemplateMissingHandling:
    ...
    def test_get_template_with_fallback_success(self, template_manager):
        ...
        template_manager.add_template(
            "decomposition", "# Decomposition Template\nContent here"
        )

        # Request a missing template that should fall back to decomposition
        result = template_manager.get_template(
            "advanced_decomposition", {}, use_default_if_missing=True
        )
    ...
    def test_get_template_with_similar_name_fallback(self, template_manager):
        ...
        result = template_manager.get_template(
            "evidence_analysis", {}, use_default_if_missing=True
        )
    ...
    def test_get_template_with_category_fallback(self, template_manager):
        ...
        result = template_manager.get_template(
            "quality_evaluation", {}, use_default_if_missing=True
        )
    ...
    def test_get_template_with_generic_fallback(self, template_manager):
        ...
        result = template_manager.get_template(
            "unknown_template", {"topic": "test topic"}, use_default_if_missing=True
        )
    ...
    def test_get_template_without_fallback_raises_error(self, template_manager):
        ...
        with pytest.raises(
            ConfigurationError, match="Template 'missing_template' not found"
        ):
            template_manager.get_template(
                "missing_template", {}, use_default_if_missing=False
            )
    ...
    def test_determine_template_type(self, template_manager):
        ...
        assert (
            template_manager._determine_template_type("analyze_step") == "step"
        ...
        assert (
            template_manager._determine_template_type("analysis_report") == "analysis"
        )
        assert (
            template_manager._determine_template_type("evaluation_form") == "evaluation"
        )
        assert (
            template_manager._determine_template_type("unknown_template") == "generic"
        )

    def test_generate_generic_analysis_template(self, template_manager):
        """Test generation of generic analysis template"""
        result = template_manager._generate_generic_analysis_template(
            "test_analysis", {"topic": "AI ethics"}
        )
    ...
    def test_generate_generic_step_template(self, template_manager):
        ...
        result = template_manager._generate_generic_step_template(
            "test_step", {"step_name": "analysis step"}
        )
    ...
    def test_generate_generic_evaluation_template(self, template_manager):
        ...
        result = template_manager._generate_generic_evaluation_template(
            "test_eval", {"content": "test content"}
        )
    ...
    def test_repair_missing_template_with_content(
        self, template_manager, temp_templates_dir
    ):
        ...
        # Repair the template
        result = template_manager.repair_missing_template(
    ...
    def test_repair_missing_template_with_generic_generation(
        self, template_manager, temp_templates_dir
    ):
    ...
    def test_repair_missing_template_reload_existing_file(
        self, template_manager, temp_templates_dir
    ):
    ...
    def test_fallback_template_logging(self, template_manager):
        ...
        with patch(
            "src.mcps.deep_thinking.templates.template_manager.logger"
        ) as mock_logger:
            template_manager.get_template(
                "advanced_decomposition", {}, use_default_if_missing=True
            )
    ...
    def test_generic_template_generation_logging(self, template_manager):
        ...
        with patch(
            ...
            template_manager.get_template(
                "completely_unknown", {}, use_default_if_missing=True
            )
    ...
    def test_fallback_mappings_coverage(self, template_manager):
        ...
        advanced_templates = [
            ...
            "advanced_critical_evaluation",
            "comprehensive_evidence_collection",
            "detailed_bias_detection",
            "enhanced_innovation",
            "deep_reflection",
        ]
        ...
        for advanced_template in advanced_templates:
            result = template_manager.get_template(
                advanced_template, {}, use_default_if_missing=True
            )
            assert result is not None
            assert len(result) > 0
            # Should not be a generic template (which would contain "不可用")
            assert "不可用" not in result or any(
                base in result for base in base_templates
            )
    ...
    def test_category_based_fallback(self, template_manager):
        ...
        for missing_template, expected_fallback in test_cases:
            result = template_manager.get_template(
                missing_template, {}, use_default_if_missing=True
            )
            # Should use the category fallback, not generate generic
            assert "不可用" not in result
            assert (
                len(result) > 10
            )  # Should be substantial content, not just placeholder

    def test_template_missing_detection_with_file_system(
        self, template_manager, temp_templates_dir
    ):
