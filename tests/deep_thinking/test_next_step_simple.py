...
def test_next_step_basic():
    ...
    next_step = flow_steps[current_index + 1] if current_index + 1 < len(flow_steps) else None
    ...
    assert selected_template == "evidence_collection", f"Expected 'evidence_collection', got {selected_template}"
    ...
    assert improvement_step == "improve_decompose", f"Expected improvement step, got {improvement_step}"
    ...
    required_fields = ["step_number", "flow_progress", "quality_gate_passed", "template_selected"]
...
def test_next_step_error_scenarios():
    ...
    if not session_exists:
        ...
        recovery_prompt = f"Session {session_id} not found. Please choose recovery option."
...
def test_next_step_template_parameters():
    ...
    if "evidence" in template_params["step_name"]:
        ...
        try:
            ...
            if sub_questions:
                ...
                template_params["keywords"] = sub_questions[0].get("search_keywords", [])
        except:
            template_params["sub_question"] = "Based on problem decomposition"
            template_params["keywords"] = ["relevant", "keywords"]
    
    assert "sub_question" in template_params, "Should have sub_question for evidence collection"
    print(f"    Evidence parameters: sub_question, keywords added")
    ...
    assert template_params["analysis_depth"] == "standard", "Should use standard depth for moderate complexity"
    print(f"    Context parameters: analysis_depth = {template_params['analysis_depth']}")
