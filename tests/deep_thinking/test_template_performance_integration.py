...
class TestTemplatePerformanceIntegration:
    ...
    def temp_templates_dir(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (templates_dir / "popular_template.tmpl").write_text(
                "Popular template: {value}"
            )
    ...
    def test_template_manager_with_performance_optimization(self, temp_templates_dir):
        ...
        manager = TemplateManager(
            temp_templates_dir, enable_performance_optimization=True
        )
        ...
        try:
            ...
            assert (
                metrics2["cache_metrics"]["total_requests"]
                > metrics["cache_metrics"]["total_requests"]
            )

        finally:
            manager.shutdown()

    def test_template_manager_without_performance_optimization(
        ...
        manager = TemplateManager(
            temp_templates_dir, enable_performance_optimization=False
        )
            ...
            for i in range(100):
                ...
                try:
                    manager.get_template(
                        template_name,
                        {"param1": f"value{i}", "param2": "test", "param3": "data"},
                    )
                except Exception:
