...
"""  # Line with trailing spaces
        ...
        readability_issues = [
            i for i in issues if i.category in ["readability", "formatting"]
        ]
...
"""
        ...
        specific_issues = self.validator.validate_content(
            specific_template, "specific_template"
        )
        vague_issues = self.validator.validate_content(vague_template, "vague_template")

        specific_spec_issues = [
            i for i in specific_issues if i.category == "specificity"
        ]
...
"""
        ...
        incomplete_issues = self.validator.validate_content(
            incomplete_decomp, "decomposition_test"
        )
        complete_issues = self.validator.validate_content(
            complete_decomp, "decomposition_test"
        )

        incomplete_comp_issues = [
            i for i in incomplete_issues if i.category == "completeness"
        ]
        complete_comp_issues = [
            i for i in complete_issues if i.category == "completeness"
        ]
...
"""
        ...
        mixed_issues = self.validator.validate_content(
            mixed_style_template, "mixed_template"
        )
        consistent_issues = self.validator.validate_content(
            consistent_template, "consistent_template"
        )

        mixed_lang_issues = [i for i in mixed_issues if i.category == "language"]
        consistent_lang_issues = [
            i for i in consistent_issues if i.category == "language"
        ]
...
"""
        ...
        issues = self.validator.validate_effectiveness(
            template_with_params, "param_template"
        )
...
"""
        ...
        with_json_issues = self.validator.validate_effectiveness(
            template_with_json, "with_json"
        )
        without_json_issues = self.validator.validate_effectiveness(
            template_without_json, "without_json"
        )

        # Template without JSON example should have more issues
        without_json_structure_issues = [
            i for i in without_json_issues if "JSON" in i.message
        ]
...
"""
        ...
        complete_issues = self.validator.validate_effectiveness(
            complete_template, "complete"
        )
        incomplete_issues = self.validator.validate_effectiveness(
            incomplete_template, "incomplete"
        )

        incomplete_instruction_issues = [
            i for i in incomplete_issues if "instruction" in i.message.lower()
        ]
...
"""
        ...
        excellent_result = self.validator.validate_template(
            excellent_template, "excellent"
        )
    ...
    def test_validation_report_generation(self):
        ...
        results = {
            ...
            "poor_template": TemplateValidationResult(
                ...
                issues=[
                    ValidationIssue(
                        ValidationSeverity.ERROR, "structure", "Missing title"
                    ),
                    ValidationIssue(
                        ValidationSeverity.WARNING, "clarity", "Vague instructions"
                    ),
                ],
                metrics={
                    "total_issues": 2,
