...
class SystemReliabilityTester:
    ...
    def test_long_term_stability(
        self, duration_minutes: int = 30, operations_per_minute: int = 60
    ) -> ReliabilityTestMetrics:
        """Test long-term system stability"""
        self.logger.info(
            f"Starting long-term stability test: {duration_minutes} minutes"
        )
                ...
                try:
                    ...
                    if operation == "create_session":
                        ...
                        success = self.session_manager.create_session(
                            session_id, f"Stability test topic {total_operations}"
                        )
                        assert success

                    elif operation == "start_thinking":
                        result = self.mcp_tools.start_thinking(
                            f"Stability test topic {total_operations}"
                        )
                        assert "session_id" in result

                    elif operation == "template_access":
                        template = self.template_manager.get_template(
                            "test_template", {"topic": f"Test {total_operations}"}
                        )
        ...
        self.logger.info(
            f"Long-term stability test completed: {successful_operations}/{total_operations} operations successful"
        )
    ...
    def test_edge_case_handling(self) -> ReliabilityTestMetrics:
        ...
        edge_cases = [
            ...
            (
                "special_chars",
                lambda: self.mcp_tools.start_thinking(
                    "测试主题 with émojis 🤔 and symbols @#$%"
                ),
            ),
            # Invalid session IDs
            (
                "invalid_session",
                lambda: self.mcp_tools.next_step("invalid_session_id", "test"),
            ),
            ("empty_session", lambda: self.mcp_tools.next_step("", "test")),
            # Malformed template parameters
            (
                "malformed_params",
                lambda: self.template_manager.get_template(
                    "test_template", {"invalid": None}
                ),
            ),
            # Database edge cases
        ...
        for case_name, test_func in edge_cases:
            ...
            try:
                ...
                if error_type in expected_errors:
                    ...
                    self.logger.debug(
                        f"Edge case {case_name} handled gracefully: {error_type}"
                    )
                else:
                    failed_operations += 1
                    error_types[error_type] = error_types.get(error_type, 0) + 1
        ...
        self.logger.info(
            f"Edge case handling test completed: {successful_operations}/{total_operations} cases handled"
        )
    ...
    def test_resource_leak_detection(self) -> ReliabilityTestMetrics:
        ...
        for cycle in range(10):  # 10 cycles of operations
            ...
            for i in range(100):
                ...
                try:
                    ...
                    result = self.mcp_tools.start_thinking(
                        f"Leak test topic {cycle}-{i}"
                    )

                    # Access templates
                    template = self.template_manager.get_template(
                        "test_template", {"topic": f"Test {i}"}
                    )
            ...
            self.logger.debug(
                f"Cycle {cycle}: Memory {cycle_end_memory:.1f}MB (+{cycle_end_memory-cycle_start_memory:.1f}), Files {cycle_end_files} (+{cycle_end_files-cycle_start_files})"
            )
        ...
        if file_handle_growth > 10:  # More than 10 file handles
            resource_leaks_detected += 1
            self.logger.warning(
                f"Potential file handle leak detected: {file_handle_growth} handles"
            )
        ...
        if len(memory_samples) >= 5:
            ...
            if recent_growth > 20:  # 20MB growth in last 5 cycles
                resource_leaks_detected += 1
                self.logger.warning(
                    f"Consistent memory growth pattern detected: {recent_growth:.1f}MB"
                )
        ...
        self.logger.info(
            f"Resource leak detection completed: {resource_leaks_detected} potential leaks detected"
        )

        return metrics

    def test_concurrent_stress(
        self, num_threads: int = 20, operations_per_thread: int = 100
    ) -> ReliabilityTestMetrics:
        """Test system under concurrent stress"""
        self.logger.info(
            f"Starting concurrent stress test: {num_threads} threads, {operations_per_thread} ops each"
        )
            ...
            for i in range(operations_per_thread):
                ...
                try:
                    ...
                    if operation_type == 0:
                        ...
                        # Template access
                        template = self.template_manager.get_template(
                            "test_template", {"topic": f"Stress {thread_id}-{i}"}
                        )
                        assert template is not None
                        ...
                        success = self.session_manager.create_session(
                            session_id, f"Stress topic {thread_id}-{i}"
                        )
                        assert success
        ...
        self.logger.info(
            f"Concurrent stress test completed: {successful_operations}/{total_operations} operations successful"
        )
    ...
    def generate_reliability_report(self) -> str:
        ...
        report_lines.extend(["INDIVIDUAL TEST RESULTS", "-" * 40])
        ...
        for metrics in self.metrics:
            ...
            recovery_rate = (
                metrics.successful_recoveries / max(metrics.recovery_attempts, 1)
    ...
    def _generate_reliability_recommendations(self) -> List[str]:
        ...
        if self.metrics:
            avg_success_rate = sum(
                m.successful_operations / max(m.total_operations, 1)
                for m in self.metrics
            ) / len(self.metrics)

            if avg_success_rate < 0.95:
                recommendations.append(
                    f"Overall success rate is {avg_success_rate:.2%} - investigate frequent failure causes"
                )
            ...
            if slow_tests:
                recommendations.append(
                    f"{len(slow_tests)} test(s) had slow response times - optimize performance"
                )
            ...
            if high_memory_tests:
                recommendations.append(
                    f"{len(high_memory_tests)} test(s) had high memory usage - check for memory leaks"
                )
            ...
            if leak_tests:
                recommendations.append(
                    f"{len(leak_tests)} test(s) detected resource leaks - investigate resource management"
                )
            ...
            poor_recovery_tests = [
                ...
                and (m.successful_recoveries / m.recovery_attempts) < 0.8
            ]
            if poor_recovery_tests:
                recommendations.append(
                    f"{len(poor_recovery_tests)} test(s) had poor error recovery - improve error handling"
                )
        ...
        if self.error_log:
            ...
            if most_common_error[1] > 5:
                recommendations.append(
                    f"Most common error is {most_common_error[0]} ({most_common_error[1]} occurrences) - prioritize fixing this"
                )

        if not recommendations:
            recommendations.append(
                "All reliability tests passed successfully - system shows good stability"
            )
...
class TestSystemReliability:
    ...
    def test_short_term_stability(self, reliability_tester):
        ...
        metrics = reliability_tester.test_long_term_stability(
            duration_minutes=1, operations_per_minute=30
        )

        # Assert basic stability requirements
        assert metrics.total_operations > 0
        assert (
            metrics.successful_operations / metrics.total_operations >= 0.9
        )  # 90% success rate
    ...
    def test_edge_case_resilience(self, reliability_tester):
        ...
        assert metrics.total_operations > 0
        assert (
            metrics.successful_operations / metrics.total_operations >= 0.8
        )  # 80% handled gracefully
    ...
    def test_concurrent_reliability(self, reliability_tester):
        ...
        metrics = reliability_tester.test_concurrent_stress(
