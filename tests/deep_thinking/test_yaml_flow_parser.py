...
class TestYAMLFlowParser:
    ...
    def test_condition_evaluation(self, yaml_parser):
        ...
        assert yaml_parser.evaluate_condition("critic.overall_score >= 0.8", context) is True
        assert yaml_parser.evaluate_condition("critic.clarity_score > 0.8", context) is False
        assert yaml_parser.evaluate_condition("decomposer.sub_questions == 3", context) is False
    ...
    def test_topological_sort(self, yaml_parser, sample_flow_yaml):
        ...
        decomposer_idx = next((i for i, step_id in enumerate(sorted_steps) 
                              if "problem_decomposition" in step_id.lower()), -1)
        evidence_idx = next((i for i, step_id in enumerate(sorted_steps) 
                            if "evidence" in step_id.lower()), -1)
                ...
                "steps": [
                    {
                        "agent": "step_a",
                        "name": "Step A",
                        "for_each": "step_c.output"
                    },
                    {
                        "agent": "step_b",
                        "name": "Step B",
                        "for_each": "step_a.output"
                    },
                    {
                        "agent": "step_c",
                        "name": "Step C",
                        "for_each": "step_b.output"
                    }
                ]
            }
        }
