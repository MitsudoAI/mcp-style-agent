...
from src.mcps.deep_thinking.models.mcp_models import (
    AnalyzeStepInput, MCPToolOutput, MCPToolName, SessionState
)
...
class TestAnalyzeStepTool:
    ...
    def test_analyze_step_decomposition_success(self, mcp_tools, mock_session_manager, 
                                              mock_template_manager, sample_session):
        ...
        mock_template_manager.get_template.return_value = "# 步骤质量分析：问题分解\n请分析以下问题分解结果..."
    ...
    def test_analyze_step_evidence_collection(self, mcp_tools, mock_session_manager, 
                                            mock_template_manager, sample_session):
        ...
        mock_session_manager.get_session.return_value = sample_session
        mock_template_manager.get_template.return_value = "# 步骤质量分析：证据收集\n请分析以下证据收集结果..."
    ...
    def test_analyze_step_debate_analysis(self, mcp_tools, mock_session_manager, 
                                        mock_template_manager, sample_session):
        ...
        mock_session_manager.get_session.return_value = sample_session
        mock_template_manager.get_template.return_value = "# 步骤质量分析：多角度辩论\n请分析以下辩论结果..."
    ...
    def test_analyze_step_format_validation_failure(self, mcp_tools, mock_session_manager, 
        ...
        mock_session_manager.get_session.return_value = sample_session
        mock_template_manager.get_template.return_value = "# 格式验证失败\n步骤结果的格式不符合要求..."
    ...
    def test_analyze_step_session_not_found(self, mcp_tools, mock_session_manager, 
        ...
        mock_template_manager.get_template.return_value = "# 会话恢复\n抱歉，之前的思考会话似乎中断了..."
    ...
    def test_analyze_step_quality_threshold_calculation(self, mcp_tools, mock_session_manager, 
                                                      mock_template_manager, sample_session):
        ...
        assert result.context["quality_threshold"] == 8.0  # critical_evaluation threshold
        
        # Test with quick_analysis flow type
        sample_session.flow_type = "quick_analysis"
        result = mcp_tools.analyze_step(input_data)
        assert result.context["quality_threshold"] == 7.5  # reduced threshold for quick analysis
    
    def test_analyze_step_improvement_suggestions_generation(self, mcp_tools, mock_session_manager, 
                                                           mock_template_manager, sample_session):
        ...
        mock_session_manager.get_session.return_value = sample_session
        mock_template_manager.get_template.return_value = "分析模板 {improvement_suggestions}"
        ...
        if result.step.startswith("analyze_"):
            ...
            assert "应包含具体的网络来源链接" in template_params["improvement_suggestions"]
        else:
            # Format validation failed, check that it was handled properly
            assert result.step.startswith("format_validation_")
            assert result.context["format_validation_failed"] is True
    
    def test_analyze_step_different_analysis_types(self, mcp_tools, mock_session_manager, 
                                                 mock_template_manager, sample_session):
    ...
    def test_analyze_step_metadata_completeness(self, mcp_tools, mock_session_manager, 
                                              mock_template_manager, sample_session):
    ...
    def test_analyze_step_error_handling(self, mcp_tools, mock_session_manager, 
                                       mock_template_manager):
...
class TestAnalyzeStepQualityGates:
    ...
    def test_quality_threshold_by_step_type(self, mcp_tools):
        ...
        assert mcp_tools._get_quality_threshold("decompose_problem", "comprehensive_analysis") == 7.0
        assert mcp_tools._get_quality_threshold("collect_evidence", "comprehensive_analysis") == 7.5
        assert mcp_tools._get_quality_threshold("critical_evaluation", "comprehensive_analysis") == 8.0
        assert mcp_tools._get_quality_threshold("reflection", "comprehensive_analysis") == 7.0
    ...
    def test_quality_threshold_by_flow_type(self, mcp_tools):
        ...
        assert mcp_tools._get_quality_threshold("decompose_problem", "quick_analysis") == 6.5
        assert mcp_tools._get_quality_threshold("critical_evaluation", "quick_analysis") == 7.5
    ...
    def test_improvement_suggestions_generation(self, mcp_tools):
        ...
        short_decomp = "简短分解"
        suggestions = mcp_tools._generate_improvement_suggestions("decompose_problem", short_decomp, context)
        assert "问题分解应更加详细" in suggestions
        assert "应为每个子问题设定优先级" in suggestions
        
        # Test evidence collection suggestions
        short_evidence = "简短证据"
        suggestions = mcp_tools._generate_improvement_suggestions("collect_evidence", short_evidence, context)
        assert "应包含具体的网络来源链接" in suggestions
        assert "应收集更多不同来源的证据" in suggestions
        
        # Test reflection suggestions
        short_reflection = "简短反思"
        suggestions = mcp_tools._generate_improvement_suggestions("reflection", short_reflection, context)
