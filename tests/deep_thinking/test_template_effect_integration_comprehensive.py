...
"""
        ...
        effect_metrics = self.effect_validator.validate_template_effect(
        ...
        assert (
            effect_metrics.effectiveness_score > 0.8
        ), f"Expected high effectiveness, got {effect_metrics.effectiveness_score}"
        assert (
            effect_metrics.structure_completeness > 0.9
        ), "Should have excellent structure"
        assert (
            effect_metrics.format_compliance > 0.8
        ...
        for param in parameters:
            assert (
                f"{{{param}}}" in test_template
            ), f"Parameter {param} should be in template"
        ...
        action_verbs = re.findall(
            r"请|需要|要求|必须|确保|运用|评估|识别|提供", test_template
        )
        assert (
            len(action_verbs) > 10
        ), f"Should have many action verbs, found {len(action_verbs)}"

        # Should have specific requirements
        specific_reqs = re.findall(r"至少\d+个|不少于|必须|确保|\d+分", test_template)
        assert (
            len(specific_reqs) > 5
        ), f"Should have specific requirements, found {len(specific_reqs)}"
        ...
        assert (
            word_count > 200
        ), f"Template should be reasonably comprehensive, got {word_count} words"
        assert (
            word_count < 2000
        ), f"Template should not be too verbose, got {word_count} words"

        # Should have multiple sections
        sections = re.findall(r"^##\s+(.+)", test_template, re.MULTILINE)
        assert (
            len(sections) >= 3
        ), f"Should have multiple sections, found {len(sections)}"

        # Should have examples and guidance (flexible check)
        has_examples = (
            "例如" in test_template
            or "比如" in test_template
            or "示例" in test_template
        )
        has_guidance = (
            "步骤" in test_template
            or "要求" in test_template
            or "框架" in test_template
        )
...
"""
        ...
        high_metrics = self.effect_validator.validate_template_effect(
            high_quality_template, "high_quality"
        )
        medium_metrics = self.effect_validator.validate_template_effect(
            medium_quality_template, "medium_quality"
        )
        low_metrics = self.effect_validator.validate_template_effect(
            low_quality_template, "low_quality"
        )

        # Verify quality ordering
        assert (
            high_metrics.effectiveness_score > medium_metrics.effectiveness_score
        ), f"High quality should be better than medium: {high_metrics.effectiveness_score} vs {medium_metrics.effectiveness_score}"

        assert (
            medium_metrics.effectiveness_score > low_metrics.effectiveness_score
        ...
        assert (
            high_metrics.effectiveness_score > 0.8
        ), "High quality template should exceed 0.8"
        assert (
            medium_metrics.effectiveness_score > 0.5
        ), "Medium quality template should exceed 0.5"
        assert (
            low_metrics.effectiveness_score < 0.8
        ), "Low quality template should be lower than high quality"

        # Test individual components (at least some should be better)
        structure_better = (
            high_metrics.structure_completeness >= medium_metrics.structure_completeness
        )
        clarity_better = (
            high_metrics.instruction_clarity_score
            > medium_metrics.instruction_clarity_score
        )
        ...
        assert (
            improvements >= 2
        ), f"High quality template should be better in at least 2/3 components, got {improvements}/3"
    ...
    def test_existing_templates_comprehensive_analysis(self):
        ...
        if report.template_metrics:
            effectiveness_scores = [
                m.effectiveness_score for m in report.template_metrics.values()
            ]
            quality_scores = [
                m.output_quality_score for m in report.template_metrics.values()
            ]
            clarity_scores = [
                m.instruction_clarity_score for m in report.template_metrics.values()
            ]

            print(f"\n📈 QUALITY METRICS ANALYSIS")
            print(
                f"Effectiveness - Min: {min(effectiveness_scores):.3f}, Max: {max(effectiveness_scores):.3f}, Avg: {sum(effectiveness_scores)/len(effectiveness_scores):.3f}"
            )
            print(
                f"Output Quality - Min: {min(quality_scores):.3f}, Max: {max(quality_scores):.3f}, Avg: {sum(quality_scores)/len(quality_scores):.3f}"
            )
            print(
                f"Instruction Clarity - Min: {min(clarity_scores):.3f}, Max: {max(clarity_scores):.3f}, Avg: {sum(clarity_scores)/len(clarity_scores):.3f}"
            )

            # Identify best and worst templates
            best_template = max(
                report.template_metrics.items(), key=lambda x: x[1].effectiveness_score
            )
            worst_template = min(
                report.template_metrics.items(), key=lambda x: x[1].effectiveness_score
            )
            ...
            print(
                f"🔧 NEEDS IMPROVEMENT: {worst_template[0]} (Score: {worst_template[1].effectiveness_score:.3f})"
            )

            # Analyze component strengths and weaknesses
            structure_scores = [
                m.structure_completeness for m in report.template_metrics.values()
            ]
            content_scores = [
                m.content_relevance for m in report.template_metrics.values()
            ]
            format_scores = [
                m.format_compliance for m in report.template_metrics.values()
            ]

            print(f"\n🔍 COMPONENT ANALYSIS")
            print(
                f"Structure Completeness - Avg: {sum(structure_scores)/len(structure_scores):.3f}"
            )
            print(
                f"Content Relevance - Avg: {sum(content_scores)/len(content_scores):.3f}"
            )
            print(
                f"Format Compliance - Avg: {sum(format_scores)/len(format_scores):.3f}"
            )

            # Verify quality standards
            high_quality_count = sum(
                1 for score in effectiveness_scores if score >= 0.8
            )
            ...
            if total_complete_templates > 0:
                ...
                print(
                    f"High Quality Templates: {high_quality_count}/{total_complete_templates} ({high_quality_ratio*100:.1f}%)"
                )

                # For complete templates, expect reasonable quality
                assert (
                    high_quality_ratio >= 0.6
                ), f"Expected at least 60% high quality templates, got {high_quality_ratio*100:.1f}%"
    ...
    def test_automated_quality_assurance_workflow(self):
        ...
        if report.tested_templates >= quality_gates["minimum_templates"]:
            print(
                f"✅ Minimum templates gate passed: {report.tested_templates} >= {quality_gates['minimum_templates']}"
            )
            gates_passed += 1
        else:
            print(
                f"❌ Minimum templates gate failed: {report.tested_templates} < {quality_gates['minimum_templates']}"
            )

        if (
            report.average_effectiveness
            >= quality_gates["minimum_average_effectiveness"]
        ):
            print(
                f"✅ Average effectiveness gate passed: {report.average_effectiveness:.3f} >= {quality_gates['minimum_average_effectiveness']}"
            )
            gates_passed += 1
        else:
            print(
                f"❌ Average effectiveness gate failed: {report.average_effectiveness:.3f} < {quality_gates['minimum_average_effectiveness']}"
            )
        ...
        if low_quality_ratio <= quality_gates["maximum_low_quality_ratio"]:
            print(
                f"✅ Low quality ratio gate passed: {low_quality_ratio:.3f} <= {quality_gates['maximum_low_quality_ratio']}"
            )
            gates_passed += 1
        else:
            print(
                f"❌ Low quality ratio gate failed: {low_quality_ratio:.3f} > {quality_gates['maximum_low_quality_ratio']}"
            )

        if report.high_quality_count >= quality_gates["minimum_high_quality_count"]:
            print(
                f"✅ High quality count gate passed: {report.high_quality_count} >= {quality_gates['minimum_high_quality_count']}"
            )
            gates_passed += 1
        else:
            print(
                f"❌ High quality count gate failed: {report.high_quality_count} < {quality_gates['minimum_high_quality_count']}"
            )
        ...
        assert (
            gates_passed >= 3
        ), f"Expected at least 3/4 quality gates to pass, got {gates_passed}/4"
