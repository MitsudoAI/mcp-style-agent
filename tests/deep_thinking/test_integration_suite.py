...
from test_system_reliability import (
    SystemReliabilityTester,
    ReliabilityTestMetrics
)
...
class IntegrationTestSuite:
    ...
    def run_complete_test_suite(self) -> IntegrationTestSuiteResults:
        ...
        try:
            ...
            quality_gates_passed, quality_gates_total, quality_gates_details = self._evaluate_quality_gates(
                end_to_end_results, concurrent_test_result, stability_metrics, reliability_metrics
            )
    ...
    def _run_end_to_end_tests(self) -> List[IntegrationTestResult]:
        ...
        for scenario in basic_scenarios:
            result = self.integration_framework.run_end_to_end_test(scenario['name'], scenario)
    ...
    def _calculate_summary_stats(self, 
                                ...
                                reliability_metrics: List[ReliabilityTestMetrics]) -> tuple:
        """Calculate summary statistics"""
        total_tests = 0
        successful_tests = 0
        failed_tests = 0
    ...
    def _evaluate_quality_gates(self,
        ...
        overall_success_rate = successful_tests / max(total_tests, 1)
        gates_details['success_rate'] = overall_success_rate >= self.config.minimum_success_rate
        ...
        gates_details['response_time'] = avg_response_time <= self.config.maximum_average_response_time
        ...
        gates_details['memory_usage'] = max_memory_usage <= self.config.maximum_memory_usage_mb
        ...
        gates_details['resource_leaks'] = total_resource_leaks <= self.config.maximum_resource_leaks
        
        # Gate 5: Stability (if stability tests were run)
        if stability_metrics:
            stability_success_rate = stability_metrics.successful_operations / max(stability_metrics.total_operations, 1)
    ...
    def _generate_recommendations(self,
                                 ...
                                 concurrent_result: Optional[IntegrationTestResult],
                                 stability_metrics: Optional[SystemStabilityMetrics],
                                 reliability_metrics: List[ReliabilityTestMetrics]) -> List[str]:
        ...
        if failed_e2e_tests:
            recommendations.append(f"{len(failed_e2e_tests)} end-to-end test(s) failed - investigate core functionality issues")
        
        # Analyze concurrent test results
        if concurrent_result and not concurrent_result.success:
            recommendations.append("Concurrent access test failed - review thread safety and resource contention")
        
        # Analyze stability metrics
        if stability_metrics:
            if stability_metrics.error_rate > 0.05:
                recommendations.append(f"High error rate in stability test ({stability_metrics.error_rate:.2%}) - improve error handling")
            
            if stability_metrics.average_response_time > 1.0:
                recommendations.append(f"Slow response times in stability test ({stability_metrics.average_response_time:.3f}s) - optimize performance")
        
        # Analyze reliability metrics
        for metrics in reliability_metrics:
            success_rate = metrics.successful_operations / max(metrics.total_operations, 1)
            if success_rate < 0.9:
                recommendations.append(f"Low success rate in {metrics.test_name} ({success_rate:.2%}) - investigate reliability issues")
            
            if metrics.resource_leaks_detected > 0:
                recommendations.append(f"Resource leaks detected in {metrics.test_name} - review resource management")
            
            if metrics.recovery_attempts > 0:
                recovery_rate = metrics.successful_recoveries / metrics.recovery_attempts
                if recovery_rate < 0.8:
                    recommendations.append(f"Poor error recovery in {metrics.test_name} ({recovery_rate:.2%}) - improve error handling")
        
        # Quality gate recommendations
        if self.results:
            failed_gates = [gate for gate, passed in self.results.quality_gates_details.items() if not passed]
            if failed_gates:
                recommendations.append(f"Quality gates failed: {', '.join(failed_gates)} - address these issues before release")
        
        if not recommendations:
            recommendations.append("All integration tests passed successfully - system is ready for deployment")
    ...
    def generate_comprehensive_report(self) -> str:
        ...
        quality_gate_status = "✅ PASSED" if self.results.quality_gates_passed == self.results.quality_gates_total else "❌ FAILED"
    ...
    def save_results(self, output_directory: Optional[str] = None):
        ...
        output_dir = Path(output_directory or self.config.output_directory or "integration_test_results")
        ...
        if self.config.save_raw_results:
            ...
            with open(output_dir / "raw_results.json", 'w', encoding='utf-8') as f:
                json.dump(results_dict, f, indent=2, default=convert_for_json, ensure_ascii=False)
...
class TestIntegrationSuite:
    ...
    def test_quick_integration_suite(self, test_suite):
        ...
        assert results.quality_gates_passed >= results.quality_gates_total * 0.8  # 80% of quality gates
    ...
    def test_comprehensive_integration_suite(self, test_suite):
        ...
        assert results.quality_gates_passed == results.quality_gates_total  # All quality gates should pass
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Deep Thinking Engine Integration Test Suite')
    ...
    try:
        ...
        print(f"Quality Gates: {results.quality_gates_passed}/{results.quality_gates_total}")
        ...
        if results.failed_tests == 0 and results.quality_gates_passed == results.quality_gates_total:
