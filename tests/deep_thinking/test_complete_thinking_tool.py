...
from src.mcps.deep_thinking.models.mcp_models import (
    CompleteThinkingInput, MCPToolOutput, MCPToolName, SessionState
)
...
class TestCompleteThinkingTool:
    ...
    def test_complete_thinking_success(self, mcp_tools, mock_session_manager, 
                                     mock_template_manager, sample_session, sample_thinking_trace):
        ...
        mock_template_manager.get_template.return_value = "Comprehensive summary template"
        
        # Create input
        input_data = CompleteThinkingInput(
            session_id="test-session-123",
            final_insights="教育质量提升需要系统性改革"
        )
        ...
        assert call_args[0][0] == "comprehensive_summary"  # First argument should be template name
    
    def test_complete_thinking_with_final_insights(self, mcp_tools, mock_session_manager, 
                                                 mock_template_manager, sample_session, sample_thinking_trace):
        ...
        final_insights = "关键洞察：教育质量的提升需要技术、制度、文化三个层面的协同改革"
        input_data = CompleteThinkingInput(
            session_id="test-session-123",
    ...
    def test_complete_thinking_session_not_found(self, mcp_tools, mock_session_manager, mock_template_manager):
    ...
    def test_complete_thinking_session_completion_failure(self, mcp_tools, mock_session_manager, 
                                                        mock_template_manager, sample_session, sample_thinking_trace):
    ...
    def test_quality_metrics_calculation(self, mcp_tools, sample_session):
        ...
        quality_metrics = mcp_tools._calculate_comprehensive_quality_metrics(sample_session)
        ...
        assert quality_metrics["overall_assessment"] == "good"  # avg 8.44 is >= 7.0 but < 8.5
    ...
    def test_session_summary_generation(self, mcp_tools, mock_session_manager, sample_session):
    ...
    def test_template_parameter_building(self, mcp_tools, sample_session, sample_thinking_trace):
        ...
        params = mcp_tools._build_comprehensive_summary_params(
            sample_session, quality_metrics, session_summary, 
            sample_thinking_trace, "Final insights"
        )
    ...
    def test_completion_instructions_generation(self, mcp_tools, sample_session):
        ...
        instructions = mcp_tools._generate_completion_instructions(excellent_metrics, sample_session)
        assert "质量优秀" in instructions
        assert "核心洞察和创新观点" in instructions
        
        # Test good quality
        good_metrics = {"overall_assessment": "good"}
        instructions = mcp_tools._generate_completion_instructions(good_metrics, sample_session)
        ...
        instructions = mcp_tools._generate_completion_instructions(poor_metrics, sample_session)
        assert "质量需要改进" in instructions
        assert "decompose_problem" in instructions
    
    def test_error_handling(self, mcp_tools, mock_session_manager, mock_template_manager):
    ...
    @pytest.mark.parametrize("quality_scores,expected_assessment", [
        ({"step1": 9.5, "step2": 9.0}, "excellent"),
        ({"step1": 8.0, "step2": 7.5}, "good"),
        ({"step1": 6.0, "step2": 5.5}, "acceptable"),
        ({"step1": 4.0, "step2": 3.5}, "needs_improvement")
    ])
    def test_overall_assessment_determination(self, mcp_tools, quality_scores, expected_assessment):
