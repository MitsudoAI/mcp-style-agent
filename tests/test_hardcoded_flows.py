...
def test_flow_step_dependencies():
    ...
    assert evidence_step is not None, "Should have collect_evidence step"
    assert (
        "decompose" in evidence_step.dependencies
...
def test_get_next_step():
    ...
    assert (
        first_step.step_id == "decompose"
    ), f"First step should be decompose, got {first_step.step_id}"
    ...
    assert (
        next_step.step_id == "collect_evidence"
    ), f"Next step should be collect_evidence, got {next_step.step_id}"

    # Verify it has for_each
    assert (
        next_step.for_each == "decompose.sub_questions"
