...
def test_flow_step_has_for_each():
    ...
    assert step.for_each == "decomposer.sub_questions", f"Expected 'decomposer.sub_questions', got {step.for_each}"
...
"""
    ...
    try:
        ...
        assert evidence_step.get('for_each') == "decomposer.sub_questions", \
...
def test_flow_executor_for_each_detection():
    ...
    assert for_each == "decomposer.sub_questions", \
...
def test_complete_integration():
    ...
    print(f"  📝 Problem decomposition generated {len(decomposition_result['sub_questions'])} sub-questions")
    ...
    step_outputs = {
        "decomposer": json.dumps(decomposition_result)
    }
    
    # Test the fix
    resolved_items = resolve_for_each_reference("decomposer.sub_questions", step_outputs, {})
    
    # Verify all sub-questions are resolved
    assert len(resolved_items) == 7, f"Expected 7 sub-questions, got {len(resolved_items)}"
    ...
    print(f"  🔧 FIXED: Previously only SQ1 was processed, now all {processed_count} are handled")
...
def main():
    ...
    try:
        ...
        print(f"✅ Fixed: System now processes {processed_count}/7 sub-questions (was 1/7)")
