...
class MCPToolOutput(BaseModel):
    ...
    session_id: Optional[str] = Field(
        default=None, description="Session identifier if applicable"
    )
    step: Optional[str] = Field(default=None, description="Current step name")
    prompt_template: str = Field(
        ..., description="Prompt template for the LLM to execute"
    )
...
class StartThinkingInput(BaseModel):
    ...
    complexity: str = Field(
        default="moderate", description="Complexity level: simple, moderate, complex"
    )
    focus: str = Field(default="", description="Specific focus or angle")
    flow_type: str = Field(
        default="comprehensive_analysis", description="Type of thinking flow to use"
    )
...
class NextStepInput(BaseModel):
    ...
    quality_feedback: Optional[Dict[str, Any]] = Field(
        default=None, description="Quality feedback if any"
    )
...
class AnalyzeStepInput(BaseModel):
    ...
    analysis_type: str = Field(
        default="quality", description="Type of analysis to perform"
    )
...
class CompleteThinkingInput(BaseModel):
    ...
    final_insights: Optional[str] = Field(
        default=None, description="Any final insights to include"
    )
...
class PromptTemplate(BaseModel):
    ...
    category: str = Field(
        ..., description="Template category (decomposition, evidence, etc.)"
    )
...
class FlowDefinition(BaseModel):
    ...
    entry_conditions: Optional[Dict[str, Any]] = Field(
        default=None, description="Conditions for using this flow"
    )
    quality_gates: Optional[Dict[str, float]] = Field(
        default=None, description="Quality thresholds for steps"
    )
...
class ErrorRecoveryPrompt(BaseModel):
    ...
    recovery_prompt: str = Field(
        ..., description="Prompt to help recover from the error"
    )
    recovery_options: List[str] = Field(
        default_factory=list, description="Available recovery options"
    )
