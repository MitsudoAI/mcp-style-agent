...
class AgentInput(BaseModel):
    ...
    parent_interaction_id: Optional[str] = Field(default=None, description="ID of parent interaction if this is a sub-task")
...
class AgentOutput(BaseModel):
    ...
    agent_type: AgentType = Field(..., description="Type of agent that produced this output")
    ...
    quality_score: Optional[float] = Field(default=None, description="Quality score of the output")
...
class AgentConfig(BaseModel):
    ...
    model_name: Optional[str] = Field(default=None, description="Specific LLM model to use")
    custom_prompts: Optional[Dict[str, str]] = Field(default=None, description="Custom prompt templates")
    quality_threshold: float = Field(default=0.8, description="Minimum quality threshold")
    specific_config: Optional[Dict[str, Any]] = Field(default=None, description="Agent-specific configuration")
...
class AgentMetadata(BaseModel):
    ...
    output_schema: Dict[str, Any] = Field(..., description="JSON schema of expected output")
    dependencies: List[AgentType] = Field(default_factory=list, description="Agent dependencies")
...
class AgentExecutionContext(BaseModel):
    ...
    previous_outputs: List[AgentOutput] = Field(default_factory=list, description="Previous agent outputs in this session")
    shared_context: Dict[str, Any] = Field(default_factory=dict, description="Shared context across agents")
    execution_mode: str = Field(default="normal", description="Execution mode (normal, debug, test)")
    resource_limits: Optional[Dict[str, Any]] = Field(default=None, description="Resource usage limits")
...
class AgentRegistration(BaseModel):
    ...
    registration_time: datetime = Field(default_factory=datetime.now, description="Registration timestamp")