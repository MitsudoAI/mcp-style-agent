...
class SearchQuery(BaseModel):
    ...
    domain_filters: List[str] = Field(default_factory=list, description="Domain restrictions")
    source_type_filters: List[SourceType] = Field(default_factory=list, description="Source type filters")
    date_range: Optional[Dict[str, str]] = Field(default=None, description="Date range for search")
    language: str = Field(default="en", description="Language preference")
    max_results: int = Field(default=10, description="Maximum number of results")
    strategy: SearchStrategy = Field(default=SearchStrategy.COMPREHENSIVE, description="Search strategy")
...
class EvidenceSource(BaseModel):
    ...
    credibility_level: CredibilityLevel = Field(..., description="Assessed credibility level")
    ...
    key_claims: List[str] = Field(default_factory=list, description="Key claims from this source")
    supporting_evidence: List[str] = Field(default_factory=list, description="Supporting evidence mentioned")
    ...
    fact_check_status: Optional[str] = Field(default=None, description="Fact-checking status if available")
    retrieved_at: datetime = Field(default_factory=datetime.now, description="When this source was retrieved")
...
class ConflictingInformation(BaseModel):
    ...
    conflict_type: str = Field(..., description="Type of conflict (factual, interpretive, etc.)")
    severity: float = Field(..., ge=0.0, le=1.0, description="Severity of conflict")
    resolution_suggestion: Optional[str] = Field(default=None, description="Suggested resolution approach")
    requires_further_investigation: bool = Field(default=False, description="Whether more investigation is needed")


class EvidenceCollection(BaseModel):
    """Collection of evidence for a specific query or topic"""
    
    query_used: SearchQuery = Field(..., description="Query that generated this collection")
    ...
    source_diversity_score: float = Field(..., ge=0.0, le=1.0, description="Diversity score of sources")
    average_credibility: float = Field(..., ge=0.0, le=1.0, description="Average credibility of sources")
    conflicting_information: List[ConflictingInformation] = Field(default_factory=list, description="Detected conflicts")
    coverage_assessment: Optional[str] = Field(default=None, description="Assessment of topic coverage")
    gaps_identified: List[str] = Field(default_factory=list, description="Identified information gaps")
    search_quality_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Overall search quality")
    collection_timestamp: datetime = Field(default_factory=datetime.now, description="When collection was created")
...
class SearchSession(BaseModel):
    ...
    raw_results: List[SearchResult] = Field(default_factory=list, description="Raw search results")
    processed_evidence: List[EvidenceCollection] = Field(default_factory=list, description="Processed evidence collections")
    search_strategy_used: SearchStrategy = Field(..., description="Overall search strategy")
    total_search_time: Optional[float] = Field(default=None, description="Total search time in seconds")
