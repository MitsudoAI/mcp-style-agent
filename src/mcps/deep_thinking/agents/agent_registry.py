...
from ..config.exceptions import AgentNotFoundError, AgentRegistrationError
...
class AgentRegistry:
    ...
    def __init__(self):
        ...
        self._agent_instances: Dict[AgentType, List[str]] = (
            {}
    ...
    async def register_agent(
        ...
        async with self._lock:
            try:
                ...
                raise AgentRegistrationError(
                    f"Failed to register agent {agent_type}: {str(e)}"
                )
    ...
    async def create_agent_instance(
        ...
        async with self._lock:
            ...
            if instance_id is None:
                instance_id = (
                    f"{agent_type}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
                )
