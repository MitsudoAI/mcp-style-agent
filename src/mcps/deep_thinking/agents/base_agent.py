...
class AgentInterface(ABC):
    ...
    @abstractmethod
    async def execute(
        self, input_data: AgentInput, context: AgentExecutionContext
    ) -> AgentOutput:
        """
...
class BaseAgent(AgentInterface):
    ...
    async def execute(
        ...
        try:
            ...
            raise AgentExecutionError(
                f"Agent {self.agent_type} execution failed: {str(e)}"
            )
        ...
        try:
            ...
            if input_data.agent_type != self.agent_type:
                raise AgentValidationError(
                    f"Agent type mismatch: expected {self.agent_type}, got {input_data.agent_type}"
                )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        successful_executions = [
            output
            for output in self.execution_history
        ...
        execution_times = [
            ...
            for output in successful_executions
...
class AgentFactory:
    ...
    def register_agent(
        ...
        if not issubclass(agent_class, BaseAgent):
            raise AgentConfigurationError("Agent class must inherit from BaseAgent")
