...
class ConfigManager:
    ...
    async def _reload_config_file(self, file_path: Path) -> None:
        ...
        async with self._lock:
            config_name = file_path.stem
            old_config = self.config_data.get(config_name, {}).copy()
    ...
    async def save_config(self, config_name: str, file_format: str = 'yaml') -> None:
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                if file_format == 'yaml':
                    yaml.dump(self.config_data[config_name], f, default_flow_style=False)
                else:
    ...
    def get_agent_config(self, agent_type: AgentType) -> AgentConfig:
        ...
        default_agent_config = {
            ...
            'quality_threshold': self.get_nested_config('agents.quality_threshold', 0.8)
        }
    ...
    def validate_config(self, config_name: str, schema: Dict[str, Any]) -> bool:
        ...
        if not isinstance(config, dict):
            raise ConfigurationError(f"Configuration '{config_name}' must be a dictionary")
