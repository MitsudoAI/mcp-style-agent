...
class HotReloadManager:
    ...
    async def _start_monitoring(self) -> None:
        ...
        try:
            ...
            logger.info(f"Started monitoring configuration directory: {self.config_dir}")
    ...
    def _get_config_name_from_path(self, file_path: Path) -> Optional[str]:
        ...
        if file_path.parent.name == "flows" and file_path.parent.parent == self.config_dir:
            return "flows"
    ...
    async def _reload_main_config_file(self, file_path: Path, config_name: str) -> None:
        ...
        try:
            ...
            merged_config = self.yaml_loader.merge_with_defaults(config_name, new_config)
            
            # Validate the configuration
            is_valid, errors = config_validator.validate_config(config_name, merged_config)
            
            if not is_valid:
                error_msg = f"Configuration validation failed for {config_name}: {'; '.join(errors)}"
                logger.error(error_msg)
                await self._notify_error_callbacks(config_name, ConfigurationError(error_msg))
                return
    ...
    async def _reload_flow_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            if not is_valid:
                ...
                logger.error(error_msg)
                await self._notify_error_callbacks(str(file_path), ConfigurationError(error_msg))
                return
    ...
    async def _notify_reload_callbacks(self, config_name: str, config_data: Dict[str, Any]) -> None:
        """
    ...
    async def force_reload(self, config_name: Optional[str] = None) -> None:
        ...
        async with self._lock:
            if config_name:
                ...
                config_files = [
                    path for path, name in self.config_file_mapping.items() 
                    if name == config_name
                ]
