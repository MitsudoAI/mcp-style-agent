...
class UserConfigManager:
    ...
    def __init__(
        ...
        self.user_config_dir = (
            Path(user_config_dir) if user_config_dir else Path.home() / ".deep_thinking"
        )
        self.global_config_dir = (
            Path(global_config_dir) if global_config_dir else Path("config")
        )
    ...
    async def _create_default_user_config(self) -> None:
        ...
        user_config_file = self.user_config_dir / "config.yaml"
        self.yaml_loader.save_config_file(
            "user_config", default_user_config, user_config_file
        )
    ...
    def get_effective_config(
        ...
        global_config = self.config_layers[ConfigPriority.GLOBAL_CONFIG].get(
            config_name, {}
        )
        effective_config = self._merge_configs(
            effective_config, global_config, config_name
        )

        # Apply user default configuration
        user_config = self.config_layers[ConfigPriority.USER_DEFAULT].get(
            config_name, {}
        )
        effective_config = self._merge_configs(
            effective_config, user_config, config_name
        )

        # Apply profile-specific configuration
        if profile in self.user_profiles:
            profile_config = (
                self.user_profiles[profile].get("config", {}).get(config_name, {})
            )
            effective_config = self._merge_configs(
                effective_config, profile_config, config_name
            )

        # Apply user custom configuration
        custom_config = self.config_layers[ConfigPriority.USER_CUSTOM].get(
            config_name, {}
        )
        effective_config = self._merge_configs(
            effective_config, custom_config, config_name
        )
        ...
        if session_overrides:
            ...
            effective_config = self._merge_configs(
                effective_config, session_config, config_name
            )
        ...
        if runtime_overrides:
            ...
            effective_config = self._merge_configs(
                effective_config, runtime_config, config_name
            )
    ...
    def _merge_configs(
        ...
        for key, value in override_config.items():
            ...
            if protected_fields and key in protected_fields:
                logger.warning(
                    f"Skipping protected field '{key}' in config '{config_name}'"
                )
                continue
            ...
            if inheritable_fields and key not in inheritable_fields:
                logger.debug(
                    f"Field '{key}' not in inheritable fields for config '{config_name}'"
                )
    ...
    def set_user_preference(
        self, key: str, value: Any, profile: Optional[str] = None
    ) -> None:
        """
    ...
    def get_user_preference(
        self, key: str, default: Any = None, profile: Optional[str] = None
    ) -> Any:
        """
    ...
    async def save_user_config(self) -> None:
        ...
        try:
            self.yaml_loader.save_config_file(
                "user_config", user_config, user_config_file
            )
    ...
    def import_config(
        self, import_path: Union[str, Path], merge_mode: str = "replace"
    ) -> None:
        """
        ...
        try:
            ...
            if "user_config" in import_data:
                if merge_mode == "replace":
                    self.config_layers[ConfigPriority.USER_DEFAULT] = import_data[
                        "user_config"
                    ]
                elif merge_mode == "merge":
                    current_config = self.config_layers[ConfigPriority.USER_DEFAULT]
                    self.config_layers[ConfigPriority.USER_DEFAULT] = (
            ...
            if "custom_config" in import_data:
                if merge_mode == "replace":
                    self.config_layers[ConfigPriority.USER_CUSTOM] = import_data[
                        "custom_config"
                    ]
                elif merge_mode == "merge":
                    current_config = self.config_layers[ConfigPriority.USER_CUSTOM]
                    self.config_layers[ConfigPriority.USER_CUSTOM] = (
                        self._deep_merge_dict(
            ...
            if "profiles" in import_data:
                ...
                if merge_mode == "replace":
                    ...
                    for profile_name, profile_data in imported_profiles.items():
                        if (
                            profile_name not in self.user_profiles
                            or merge_mode == "replace"
                        ):
    ...
    def validate_user_config(
        self, config_name: str, profile: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_config_diff(
        self, config_name: str, profile1: str, profile2: str
    ) -> Dict[str, Any]:
        """
    ...
    def _compute_config_diff(
        self, config1: Dict[str, Any], config2: Dict[str, Any], comparison_name: str
    ) -> Dict[str, Any]:
