    ...
    def parse_file(self, file_path: Union[str, Path]) -> Dict[str, ThinkingFlow]:
        ...
        try:
            ...
            if not file_path.exists():
                raise FlowConfigurationError(f"Flow configuration file not found: {file_path}")
                
            with open(file_path, 'r', encoding='utf-8') as f:
                yaml_content = yaml.safe_load(f)
                
            if not isinstance(yaml_content, dict):
                raise FlowConfigurationError(f"Invalid YAML format in {file_path}, expected dictionary")
            ...
            raise FlowConfigurationError(f"Error processing flow configuration file {file_path}: {e}")
    ...
    def _parse_flow(self, flow_name: str, flow_data: Dict[str, Any]) -> ThinkingFlow:
        ...
        try:
            ...
            if not isinstance(flow_data['steps'], list):
                raise FlowConfigurationError(f"Flow '{flow_name}' 'steps' must be a list")
            ...
            for i, step_data in enumerate(flow_data['steps']):
                ...
                step_id = step_data.get('name', f"{flow_name}_step_{i}").lower().replace(' ', '_')
        ...
        for step_id in dependency_graph:
            if step_id not in visited:
                if has_cycle(step_id):
                    raise FlowConfigurationError(f"Cycle detected in flow '{flow.name}' dependencies")
