...
class FlowStepSchema(BaseModel):
    ...
    def validate_for_each(cls, v):
        if v is not None and not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*$', v):
...
class ConfigValidator:
    ...
    def validate_system_config(self, config_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
    ...
    def validate_flows_config(self, config_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
    ...
    def _validate_single_flow(self, flow_name: str, flow_config: Dict[str, Any]) -> List[str]:
        """
    ...
    def _validate_flow_steps(self, flow_name: str, steps: List[Dict[str, Any]]) -> List[str]:
        """
        Validate flow steps
        ...
        for i, step in enumerate(steps):
            ...
            if step_name in step_names:
                errors.append(f"Flow '{flow_name}' has duplicate step name: {step_name}")
            step_names.add(step_name)
            
            # Validate agent type
            if not agent_type:
                errors.append(f"Flow '{flow_name}' step '{step_name}' missing agent type")
                continue
            ...
            if for_each:
                ref_errors = self._validate_reference(flow_name, step_name, for_each, agent_outputs)
            ...
            if repeat_until:
                ...
                if "." in repeat_until:
                    ...
                    import re
                    refs = re.findall(r'([a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*)', repeat_until)
                    for ref in refs:
                        ref_errors = self._validate_reference(flow_name, step_name, ref, agent_outputs)
                        errors.extend(ref_errors)
            
            # Validate step configuration
            step_config = step.get("config", {})
            config_errors = self._validate_step_config(flow_name, step_name, agent_type, step_config)
    ...
    def _validate_step_config(
        ...
        if agent_type == "decomposer":
            ...
            if max_questions is not None:
                if not isinstance(max_questions, int) or max_questions < 1 or max_questions > 10:
                    errors.append(
            ...
            if min_sources is not None:
                if not isinstance(min_sources, int) or min_sources < 1 or min_sources > 20:
                    errors.append(
            ...
            if credibility_threshold is not None:
                if not isinstance(credibility_threshold, (int, float)) or not 0 <= credibility_threshold <= 1:
                    errors.append(
        ...
        for agent_type in self.required_agent_types:
            if agent_type in agents_config:
                ...
                if not isinstance(agent_config, dict):
                    errors.append(f"Agent '{agent_type}' configuration must be a dictionary")
                    continue
                
                # Validate agent-specific settings
                agent_errors = self._validate_agent_specific_config(agent_type, agent_config)
                errors.extend(agent_errors)
        
        return errors
    
    def _validate_agent_specific_config(self, agent_type: str, config: Dict[str, Any]) -> List[str]:
        """
        ...
        if agent_type == "critic":
            ...
            if standards and not isinstance(standards, list):
                errors.append(f"Agent '{agent_type}': paul_elder_standards must be a list")
            
            valid_standards = {
                "accuracy", "precision", "relevance", "logic", "breadth", 
                "depth", "significance", "fairness", "clarity"
            }
            
            if standards:
                for standard in standards:
                    if standard not in valid_standards:
                        errors.append(f"Agent '{agent_type}': invalid Paul-Elder standard '{standard}'")
            ...
            valid_bias_types = {
                ...
                "representativeness", "overconfidence", "hindsight_bias"
            }
            
            if bias_types:
                for bias_type in bias_types:
                    if bias_type not in valid_bias_types:
                        errors.append(f"Agent '{agent_type}': invalid bias type '{bias_type}'")
            ...
            valid_methods = {"SCAMPER", "TRIZ", "lateral_thinking", "analogical_reasoning"}
            
            if methods:
                for method in methods:
                    if method not in valid_methods:
                        errors.append(f"Agent '{agent_type}': invalid innovation method '{method}'")
    ...
    def _validate_error_handling(self, flow_name: str, error_config: Dict[str, Any]) -> List[str]:
        """
        Validate error handling configuration
        ...
        valid_strategies = {"exponential_backoff", "linear_backoff", "fixed_delay", "none"}
        ...
        if fallback_agents and not isinstance(fallback_agents, list):
            errors.append(
                f"Flow '{flow_name}': fallback_agents must be a list"
            )
    ...
    def get_validation_summary(self, config_name: str, config_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if config_name == "flows":
            summary["flow_count"] = len([k for k in config_data.keys() if not k.startswith("_")])
