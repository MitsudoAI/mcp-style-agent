...
class FlowUpdateAnalyzer:
    ...
    def analyze_flow_changes(
        self, old_flow: ThinkingFlow, new_flow: ThinkingFlow
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_step_changes(
        ...
        new_order = [
            step.step_id for step in new_flow.steps if step.step_id in old_steps
        ]
    ...
    def _analyze_single_step_changes(
        self, old_step: FlowStep, new_step: FlowStep
    ) -> List[Dict[str, Any]]:
...
class ActiveSessionMigrator:
    ...
    async def migrate_active_sessions(
        ...
        """
        logger.info(
            f"Starting session migration for flow '{flow_name}' using strategy '{migration_strategy}'"
        )
        ...
        if not migration_func:
            raise ConfigurationError(
                f"Unknown migration strategy: {migration_strategy}"
            )
    ...
    async def _hot_update_migration(
        ...
        for session in active_sessions:
            try:
                ...
                logger.error(
                    f"Failed to hot update session {session.get('session_id')}: {e}"
                )
    ...
    async def _graceful_migration(
        ...
        for session in active_sessions:
            try:
                ...
                logger.error(
                    f"Failed to gracefully migrate session {session.get('session_id')}: {e}"
                )
        ...
        for session in active_sessions:
            try:
                ...
                await self._restore_session_state(
                    new_session_id, session_state, new_flow
                )
                ...
                logger.error(
                    f"Failed to restart session {session.get('session_id')}: {e}"
                )
    ...
    async def _wait_for_step_completion(
        ...
        while True:
            ...
            if asyncio.get_event_loop().time() - start_time > timeout:
                logger.warning(
                    f"Timeout waiting for step completion in session {session_id}"
                )
                break
    ...
    async def _execute_session_migration_plan(
        self, session_id: str, migration_plan: Dict[str, Any]
    ) -> None:
        ...
        for action in migration_plan["actions"]:
            ...
            if action_type == "skip_to_next_step":
                ...
                await self._evaluate_new_step_for_session(session_id, action["step_id"])
    ...
    async def _update_session_flow_cache(
        self, session_id: str, new_flow: ThinkingFlow
    ) -> None:
...
class FlowHotUpdateManager:
    ...
    def __init__(self, flow_config_manager, session_manager):
    ...
    async def _handle_flow_update(
        self, flow_name: str, old_flow: ThinkingFlow, new_flow: ThinkingFlow
    ) -> None:
        ...
        if analysis["migration_required"]:
            migration_results = await self.migrator.migrate_active_sessions(
                flow_name, old_flow, new_flow, analysis["migration_strategy"], analysis
            )
