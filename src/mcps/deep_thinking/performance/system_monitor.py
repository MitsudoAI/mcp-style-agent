...
class PerformanceBottleneckDetector:
    ...
    def analyze_response_times(
        self, response_stats: Dict[str, ResponseTimeStats]
    ) -> List[str]:
    ...
    def _generate_recommendations(self, bottlenecks: List[str]) -> List[str]:
        ...
        for bottleneck in bottlenecks:
            if "CPU usage" in bottleneck:
                ...
                recommendations.append(
                    "Review memory usage patterns and implement garbage collection"
                )
            elif "disk usage" in bottleneck:
                recommendations.append(
                    "Clean up temporary files or increase disk space"
                )
...
class ResponseTimeTracker:
    ...
    def record_response_time(
        self, operation_name: str, response_time: float, success: bool = True
    ):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time is not None:
            ...
            self.tracker.record_response_time(
                self.operation_name, response_time, self.success
            )
...
class SystemResourceMonitor:
    ...
    def start_monitoring(self):
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="system-resource-monitor"
        )
    ...
    def get_historical_stats(self, minutes: int = 10) -> List[SystemResourceStats]:
        ...
        with self.lock:
            return [
                stats
                for stats in self.resource_history
...
class SystemPerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_resources": {
                ...
                "disk_usage_percent": (
                    current_stats.disk_usage_percent if current_stats else 0
                ),
                "process_memory_mb": (
                    current_stats.process_memory_mb if current_stats else 0
                ),
    ...
    def generate_performance_report(self) -> str:
        ...
        if trends:
            ...
            report.append(
                f"  Bottleneck Frequency: {trends.get('bottleneck_frequency', 0):.1%}"
            )
