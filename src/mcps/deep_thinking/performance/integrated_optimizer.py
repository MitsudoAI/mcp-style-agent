...
class IntegratedPerformanceOptimizer:
    ...
    def __init__(
        ...
        if enable_template_optimization:
            try:
                ...
                self.template_optimizer = TemplatePerformanceOptimizer(
                    Path(templates_dir), cache_size=100, cache_memory_mb=50
                )
        ...
        if enable_database_optimization and db_path != ":memory:":
            try:
                self.database_optimizer = DatabasePerformanceOptimizer(
                    db_path, min_connections=2, max_connections=10
                )
        ...
        if enable_system_monitoring:
            try:
                ...
                self.system_monitor.add_optimization_callback(
                    self._handle_performance_issues
                )
    ...
    def get_template(
        self, name: str, load_func: Callable[[str], Optional[str]]
    ) -> Optional[str]:
    ...
    def optimize_all_systems(self):
        ...
        with self.lock:
            ...
            try:
                ...
                logger.info(
                    f"Comprehensive optimization completed in {optimization_time:.2f}s"
                )
    ...
    def _handle_performance_issues(self, bottleneck_result: Dict[str, Any]):
        ...
        try:
            ...
            logger.warning(
                f"Performance issues detected: {bottleneck_count} bottlenecks, severity: {severity}"
            )
            ...
            if severity in ["medium", "high"]:
                ...
                if any(
                    "memory" in str(b)
                    for b in bottleneck_result.get("resource_bottlenecks", [])
                ):
                    if self.template_optimizer:
                        self.template_optimizer.optimize_cache()
                        logger.info(
                            "Applied template cache optimization for memory issues"
                        )

                # Database optimization for slow queries
                if any(
                    "Slow operation" in str(b)
                    for b in bottleneck_result.get("response_bottlenecks", [])
                ):
                    if self.database_optimizer:
                ...
                if severity == "high":
                    if self.system_monitor:
                        self.system_monitor.optimize_system_performance()
                        logger.info(
                            "Applied system optimization for high severity issues"
                        )
    ...
    def generate_performance_report(self) -> str:
        ...
        report.append(
            f"  Template Optimization: {'Enabled' if optimizer_info['template_optimizer_enabled'] else 'Disabled'}"
        )
        report.append(
            f"  Database Optimization: {'Enabled' if optimizer_info['database_optimizer_enabled'] else 'Disabled'}"
        )
        report.append(
            f"  System Monitoring: {'Enabled' if optimizer_info['system_monitor_enabled'] else 'Disabled'}"
        )
        ...
        if (
            "template_performance" in metrics
            and "error" not in metrics["template_performance"]
        ):
            ...
            if "cache_metrics" in template_perf:
                ...
                report.append(
                    f"  Cache Hit Rate: {cache_metrics.get('hit_rate', 0):.1%}"
                )
                report.append(
                    f"  Cache Size: {cache_metrics.get('cache_size', 0)} templates"
                )
                report.append(
                    f"  Memory Usage: {cache_metrics.get('cache_memory_usage_mb', 0):.1f}MB"
                )
                report.append("")

        # Database performance
        if (
            "database_performance" in metrics
            and "error" not in metrics["database_performance"]
        ):
            ...
            if "connection_pool" in db_perf:
                ...
                report.append(
                    f"  Active Connections: {pool_metrics.get('active_connections', 0)}"
                )
                report.append(
                    f"  Pool Utilization: {pool_metrics.get('pool_utilization', 0):.1%}"
                )
                report.append(
                    f"  Average Wait Time: {pool_metrics.get('average_wait_time', 0):.3f}s"
                )
                report.append("")
        ...
        if (
            ...
            and "error" not in metrics["system_performance"]
        ):
            ...
            if "system_resources" in sys_perf:
                ...
                report.append(
                    f"  Process Memory: {resources.get('process_memory_mb', 0):.1f}MB"
                )
                report.append("")
            ...
            if sys_perf.get("recent_bottlenecks"):
                ...
                for bottleneck in sys_perf["recent_bottlenecks"][-3:]:
                    report.append(
                        f"  {bottleneck['timestamp']} - Severity: {bottleneck['severity']}"
                    )
        ...
        if self.optimization_history:
            ...
            for opt in self.optimization_history[-5:]:  # Last 5 optimizations
                ...
                report.append(
                    f"  {timestamp} - {', '.join(opt['optimizations_applied'])}"
                )
            report.append("")
