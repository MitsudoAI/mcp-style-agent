...
class FlowStateMachine:
    ...
    def __init__(self, db: Optional[ThinkingDatabase] = None):
        ...
        self._auto_recovery_enabled = True  # Enable automatic recovery of interrupted flows
        self._max_retry_attempts = 3  # Maximum number of retry attempts for failed steps
        logger.info("FlowStateMachine initialized with persistence: %s", "enabled" if db else "disabled")
    ...
    def transition(
        self, flow: ThinkingFlow, event: FlowEvent, metadata: Optional[Dict[str, Any]] = None
        ...
        if current_state not in self._valid_transitions or event not in self._valid_transitions[current_state]:
            # Handle special cases
            if event == FlowEvent.MANUAL_OVERRIDE and metadata and "target_state" in metadata:
                ...
                self._record_state_change(flow.flow_id, current_state, target_state, event, metadata)
        ...
        try:
            ...
            self._record_state_change(flow.flow_id, current_state, new_state, event, metadata)
    ...
    def _record_state_change(
        ...
        if to_state in [FlowStatus.FAILED, FlowStatus.CANCELLED]:
            logger.warning(f"Flow {flow_id} state change: {from_state} -> {to_state} via {event}")
        else:
            logger.info(f"Flow {flow_id} state change: {from_state} -> {to_state} via {event}")
    ...
    def _perform_state_actions(
        ...
        try:
            ...
            if new_state == FlowStatus.RUNNING:
                if event == FlowEvent.START:
                    ...
                    if flow.flow_id in self._paused_flows:
                        ...
                        logger.info(f"Resumed flow {flow.flow_id} after {pause_duration:.1f}s pause")
                    ...
                    if error_step and error_step in flow.steps:
                        ...
                        logger.info(f"Recovered flow {flow.flow_id} by resetting step {error_step}")
                    else:
            ...
            if event == FlowEvent.COMPLETE_STEP and metadata and "step_id" in metadata:
                ...
                if step_id in flow.steps:
                    ...
                    if all_complete:
                        ...
                        logger.info(f"All steps complete in flow {flow.flow_id}, auto-completing flow")
                        self.transition(flow, FlowEvent.COMPLETE, {"auto_completed": True})
                else:
                ...
                if step_id in flow.steps:
                    ...
                    logger.warning(f"Failed step {step_id} in flow {flow.flow_id}: {error_message}")
                    ...
                    if not step.can_retry() and metadata.get("critical", False):
                        ...
                        self.transition(flow, FlowEvent.ERROR, {
                ...
                if step_id in flow.steps:
                    ...
                    if step.status == FlowStepStatus.FAILED and step.can_retry():
                        step.status = FlowStepStatus.PENDING
                        logger.info(f"Retrying step {step_id} in flow {flow.flow_id} (attempt {step.retry_count + 1})")
                    else:
                        logger.warning(
                ...
                if step_id in flow.steps:
                    ...
                    logger.info(f"Skipped step {step_id} in flow {flow.flow_id}: {skip_reason}")
                else:
                    logger.warning(f"Step {step_id} not found in flow {flow.flow_id}")
            ...
            logger.error(f"Error performing state actions for flow {flow.flow_id}: {e}", exc_info=True)
            return False
    ...
    def _persist_state(self, flow: ThinkingFlow) -> bool:
        ...
        try:
                ...
                self.db.update_session(
                    ...
                    quality_metrics={
                        ...
                        "state_machine_stats": {
                            ...
                            "step_counts": {
                                status.value: len([s for s in flow.steps.values() if s.status == status])
            ...
            logger.error(f"Error persisting flow state for {flow.flow_id}: {e}", exc_info=True)
            return False

    def restore_flow_state(self, flow_id: str, session_id: str) -> Optional[ThinkingFlow]:
        """
        ...
        try:
            ...
            logger.info(f"Restoring flow state version {persistence_version} for {flow_id}")
            ...
            try:
                ...
                logger.warning(f"Invalid flow status in database: {flow_state.get('status')}, using INITIALIZED")
            ...
            if "start_time" in flow_state and flow_state["start_time"]:
                try:
                    ...
                    logger.warning(f"Invalid start_time format in flow {flow_id}, ignoring")
                
            if "end_time" in flow_state and flow_state["end_time"]:
                try:
                    flow.end_time = datetime.fromisoformat(flow_state["end_time"])
                except (ValueError, TypeError):
                    logger.warning(f"Invalid end_time format in flow {flow_id}, ignoring")
            ...
            for step_id, step_data in flow_state.get("steps", {}).items():
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid step status in database: {step_data.get('status')}, using PENDING")
                        step.status = FlowStepStatus.PENDING
                    ...
                    if "start_time" in step_data and step_data["start_time"]:
                        try:
                            ...
                            logger.warning(f"Invalid start_time format in step {step_id}, ignoring")
                        
                    if "end_time" in step_data and step_data["end_time"]:
                        try:
                            step.end_time = datetime.fromisoformat(step_data["end_time"])
                        except (ValueError, TypeError):
                            logger.warning(f"Invalid end_time format in step {step_id}, ignoring")
                    
                    flow.add_step(step)
                    steps_restored += 1
                    
                except Exception as step_error:
                    logger.error(f"Error restoring step {step_id} in flow {flow_id}: {step_error}")
            
            # Restore step order
            flow.step_order = flow_state.get("step_order", [])
            
            # Restore state history
            if "quality_metrics" in session_data and "flow_state_history" in session_data["quality_metrics"]:
                try:
                    history = session_data["quality_metrics"]["flow_state_history"]
                    self._state_history[flow_id] = history
                    logger.info(f"Restored {len(history)} state transitions for flow {flow_id}")
                except Exception as history_error:
                    logger.error(f"Error restoring state history for flow {flow_id}: {history_error}")
            
            logger.info(f"Successfully restored flow {flow_id} with {steps_restored} steps")
            ...
            logger.error(f"Error restoring flow state for {flow_id}: {e}", exc_info=True)
            return None
    ...
    def reset_flow(self, flow: ThinkingFlow) -> bool:
        ...
        try:
            ...
            logger.info(f"Reset flow {flow.flow_id} from {original_state} to {FlowStatus.INITIALIZED}")
            return True
    ...
    def pause_flow(self, flow: ThinkingFlow, reason: str = "User requested") -> bool:
        ...
        try:
            if flow.status != FlowStatus.RUNNING:
                logger.warning(f"Cannot pause flow {flow.flow_id}: not in RUNNING state (current: {flow.status})")
                return False
    ...
    def resume_flow(self, flow: ThinkingFlow) -> bool:
        ...
        try:
            if flow.status != FlowStatus.PAUSED:
                logger.warning(f"Cannot resume flow {flow.flow_id}: not in PAUSED state (current: {flow.status})")
                return False
    ...
    def cancel_flow(self, flow: ThinkingFlow, reason: str = "User cancelled") -> bool:
        ...
        try:
            ...
            if flow.status not in [FlowStatus.RUNNING, FlowStatus.PAUSED, FlowStatus.INITIALIZED]:
                logger.warning(
    ...
    def check_for_timeouts(self, timeout_minutes: int = 60) -> List[str]:
        ...
        for flow_id, pause_time in list(self._paused_flows.items()):
            if now - pause_time > timeout_threshold:
                logger.warning(f"Flow {flow_id} timed out after {timeout_minutes} minutes of inactivity")
    ...
    def get_flow_state_summary(self, flow: ThinkingFlow) -> Dict[str, Any]:
        ...
        try:
            ...
            for entry in history:
                ...
                history_stats["state_counts"][to_state] = history_stats["state_counts"].get(to_state, 0) + 1
            ...
            return {
                ...
                "progress_percentage": (
                    (step_counts[FlowStepStatus.COMPLETED.value] + step_counts[FlowStepStatus.SKIPPED.value]) 
                    / len(flow.steps) * 100
                    if len(flow.steps) > 0
                    else 0
                ),
                ...
                "valid_transitions": [event.value for event in valid_transitions.keys()],
            ...
            logger.error(f"Error generating flow state summary for {flow.flow_id}: {e}", exc_info=True)
            return {
    ...
    def get_step_state_summary(self, flow: ThinkingFlow, step_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error generating step state summary for {step_id} in flow {flow.flow_id}: {e}", exc_info=True)
            return {
    ...
    def get_flow_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_transitions = sum(len(history) for history in self._state_history.values()) / len(self._state_history) if self._state_history else 0
    ...
    def cleanup_history(self, max_age_days: int = 30) -> int:
        ...
        try:
            ...
            logger.info(f"Cleaned up history for {flows_cleaned} flows older than {max_age_days} days")
        ...
        try:
            ...
            for i in range(1, len(history)):
                ...
                try:
                    ...
                    state_durations[state]["min_seconds"] = min(state_durations[state]["min_seconds"], duration)
                    state_durations[state]["max_seconds"] = max(state_durations[state]["max_seconds"], duration)
            ...
            logger.error(f"Error exporting flow history for {flow_id}: {e}", exc_info=True)
            return {
