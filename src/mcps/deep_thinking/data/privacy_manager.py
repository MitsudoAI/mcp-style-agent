...
class PrivacyManager:
    ...
    def __init__(self, data_directory: Optional[str] = None):
        ...
        logger.info(f"PrivacyManager initialized with data directory: {self.data_directory}")
    ...
    def verify_data_localization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not verification_results["directory_writable"]:
                verification_results["data_localization_verified"] = False
                verification_results.setdefault("issues", []).append("Data directory is not writable")
    ...
    def create_encrypted_database(self, db_name: str = "sessions.db") -> ThinkingDatabase:
        """
    ...
    def complete_data_deletion(self) -> Dict[str, Any]:
        ...
        try:
            if not self.data_directory.exists():
                ...
                deletion_results["message"] = "No data directory found - nothing to delete"
            ...
            try:
                ...
                deletion_results["deletion_successful"] = True
                deletion_results["message"] = f"Successfully deleted {len(deletion_results['files_deleted'])} files"
            except Exception as e:
    ...
    def selective_data_deletion(
        ...
        try:
            ...
            deletion_results["deletion_successful"] = len(deletion_results["sessions_failed"]) == 0
            deletion_results["message"] = f"Deleted {len(deletion_results['sessions_deleted'])} sessions"
        ...
        try:
            ...
            if db_path.exists():
                if include_encrypted:
                    ...
                    export_results["total_size_exported"] += export_db_path.stat().st_size
                else:
                    ...
                    if anonymize:
                        ...
                        for session in sessions:
                            session["topic"] = f"[ANONYMIZED_TOPIC_{hash(session['topic']) % 10000}]"
                    ...
                    export_results["total_size_exported"] += sessions_file.stat().st_size
            
            export_results["export_successful"] = True
            export_results["message"] = f"Exported {len(export_results['files_exported'])} files"
        ...
        try:
            ...
            if self.data_directory.exists():
                ...
                if dir_stat.st_mode & 0o044:  # Others can read
                    ...
                    compliance_results["recommendations"].append("Restrict file permissions")
            
            # Update overall compliance
            compliance_results["overall_compliance"] = len(compliance_results["issues_found"]) == 0
    ...
    def get_data_usage_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_report["summary"] = {
                ...
                "total_size_mb": round(usage_report["total_size_bytes"] / (1024 * 1024), 2),
                "largest_file": max(
                    usage_report["file_breakdown"], 
                    key=lambda x: x["size_bytes"]
                ) if usage_report["file_breakdown"] else None
            }
