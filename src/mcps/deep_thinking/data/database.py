...
class ThinkingDatabase:
    ...
    def create_session(self, session_id: str, topic: str, session_type: str = "comprehensive_analysis",
                      user_id: Optional[str] = None, configuration: Optional[Dict[str, Any]] = None) -> bool:
        """Create a new thinking session"""
        try:
            with self.get_connection() as conn:
                # Encrypt sensitive data
                topic_encrypted = self._encrypt_if_enabled(topic) if self.encryption else None
                ...
                """, (
                    session_id,
                    user_id,
                    topic if not self.encryption else "",  # Store empty if encrypted
                    topic_encrypted,
                    session_type,
                    config_json,
        ...
        try:
            with self.get_connection() as conn:
                ...
                if self.encryption and session_data.get('topic_encrypted'):
                    session_data['topic'] = self._decrypt_if_enabled(session_data['topic_encrypted'])
                
                if session_data.get('configuration'):
                    session_data['configuration'] = self._decrypt_json_if_enabled(session_data['configuration'])
                
                if session_data.get('context'):
                    session_data['context'] = self._decrypt_json_if_enabled(session_data['context'])
                
                if session_data.get('quality_metrics'):
                    session_data['quality_metrics'] = self._decrypt_json_if_enabled(session_data['quality_metrics'])
    ...
    def update_session(self, session_id: str, **updates) -> bool:
        ...
        try:
            ...
            for key, value in updates.items():
                if key in ['context', 'quality_metrics', 'configuration'] and isinstance(value, dict):
    ...
    def add_session_step(self, session_id: str, step_name: str, step_number: int,
                        step_type: str, template_used: Optional[str] = None,
            ...
            with self.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if step_data.get('input_data'):
                        step_data['input_data'] = self._decrypt_json_if_enabled(step_data['input_data'])
                    if step_data.get('output_data'):
                        step_data['output_data'] = self._decrypt_json_if_enabled(step_data['output_data'])
    ...
    def add_step_result(self, session_id: str, step_id: int, result_type: str,
                       content: str, metadata: Optional[Dict[str, Any]] = None,
                       quality_indicators: Optional[Dict[str, Any]] = None,
                       citations: Optional[List[Dict[str, Any]]] = None) -> Optional[int]:
    ...
    def get_step_results(self, session_id: str, step_id: Optional[int] = None) -> List[Dict[str, Any]]:
            ...
            with self.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if result_data.get('content'):
                        result_data['content'] = self._decrypt_if_enabled(result_data['content'])
    ...
    def list_sessions(self, user_id: Optional[str] = None, status: Optional[str] = None,
    ...
    def delete_session(self, session_id: str) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor = conn.execute("DELETE FROM thinking_sessions WHERE id = ?", (session_id,))
                conn.commit()
