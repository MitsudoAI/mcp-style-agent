...
class TemplatePreloader:
    ...
    def __init__(self, templates_dir: Path, cache: LRUCache):
        ...
        self.cache = cache
        self.preload_stats = {}
        self.preload_executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="template-preload")
        self.lock = threading.RLock()
    ...
    def preload_templates(self, template_names: Optional[List[str]] = None) -> Dict[str, bool]:
        ...
        try:
            ...
            for name in template_names:
                future = self.preload_executor.submit(self._preload_single_template, name)
            ...
            for future in as_completed(future_to_name):
                ...
                try:
                    success = future.result(timeout=10.0)  # 10 second timeout per template
            ...
            logger.info(f"Preloaded {success_count}/{len(template_names)} templates in {preload_time:.2f}s")
            ...
            with self.lock:
                self.preload_stats = {
                    ...
                    'success_rate': success_count / len(template_names) if template_names else 0.0
                }
...
class MemoryMonitor:
    ...
    def __init__(self, warning_threshold_mb: int = 100, critical_threshold_mb: int = 200):
        ...
        while self.monitoring_active:
            try:
                ...
                if current_usage > self.critical_threshold:
                    logger.warning(f"Critical memory usage: {current_usage / 1024 / 1024:.1f}MB")
                    self._trigger_cleanup(aggressive=True)
                elif current_usage > self.warning_threshold:
                    logger.info(f"High memory usage: {current_usage / 1024 / 1024:.1f}MB")
    ...
    def _trigger_cleanup(self, aggressive: bool = False):
        ...
        try:
            logger.info(f"Triggering {'aggressive' if aggressive else 'normal'} memory cleanup")
...
class TemplatePerformanceOptimizer:
    ...
    def __init__(self, templates_dir: Path, cache_size: int = 100, cache_memory_mb: int = 50):
        self.templates_dir = templates_dir
        ...
        try:
            with self.lock:
                ...
                if content is not None:
                    ...
                    self._update_usage_stats(name, time.time() - start_time, cache_hit=True)
                    return content
                ...
                if content is not None:
                    ...
                    self._update_usage_stats(name, time.time() - start_time, cache_hit=False)
                    return content
    ...
    def _calculate_priority_score(self, stats: TemplateUsageStats) -> float:
        ...
        if stats.last_accessed:
            hours_since_access = (datetime.now() - stats.last_accessed).total_seconds() / 3600
    ...
    def preload_high_priority_templates(self, max_templates: int = 20) -> Dict[str, bool]:
        ...
        if not high_priority_names:
            ...
            high_priority_names = [
                'decomposition', 'evidence_collection', 'critical_evaluation',
                'bias_detection', 'innovation', 'reflection'
            ]
    ...
    def optimize_cache(self):
        ...
        try:
            with self.lock:
                ...
                logger.info(f"Cache optimization complete. Evicted {len(templates_to_evict)} templates")
    ...
    def _memory_cleanup_callback(self, aggressive: bool):
        ...
        try:
            if aggressive:
                ...
                cached_templates = list(self.cache.cache.keys())
                template_priorities = [
                    (name, self.usage_stats.get(name, TemplateUsageStats(name)).priority_score)
                    for name in cached_templates
                ]
                template_priorities.sort(key=lambda x: x[1])  # Sort by priority (ascending)
                ...
                logger.info(f"Aggressive cleanup: removed {len(templates_to_remove)} templates from cache")
            else:
