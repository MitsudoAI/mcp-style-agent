...
class TemplateFormatValidator:
    ...
    def validate_format(self, template_content: str, template_name: str) -> List[ValidationIssue]:
    ...
    def _check_required_sections(self, content: str, template_name: str) -> List[ValidationIssue]:
        ...
        if template_name not in ['bias_detection', 'innovation', 'reflection']:  # Simple templates
...
class TemplateContentValidator:
    ...
    def validate_content(self, template_content: str, template_name: str) -> List[ValidationIssue]:
    ...
    def _check_instruction_clarity(self, content: str) -> List[ValidationIssue]:
        ...
        positive_count = sum(content.count(word) for word in self.clarity_indicators['positive'])
        negative_count = sum(content.count(word) for word in self.clarity_indicators['negative'])
    ...
    def _check_specificity(self, content: str) -> List[ValidationIssue]:
        ...
        specific_reqs = re.findall(self.instruction_patterns['specific_requirements'], content)
        ...
        vague_count = sum(len(re.findall(pattern, content)) for pattern in vague_patterns)
    ...
    def _check_completeness(self, content: str, template_name: str) -> List[ValidationIssue]:
...
class TemplateEffectivenessValidator:
    ...
    def validate_effectiveness(self, template_content: str, template_name: str) -> List[ValidationIssue]:
    ...
    def _test_output_structure(self, content: str, template_name: str) -> List[ValidationIssue]:
...
class TemplateValidator:
    ...
    def validate_template(self, template_content: str, template_name: str) -> TemplateValidationResult:
        """Perform comprehensive template validation"""
        all_issues = []
        
        # Run all validators
        format_issues = self.format_validator.validate_format(template_content, template_name)
        content_issues = self.content_validator.validate_content(template_content, template_name)
        effectiveness_issues = self.effectiveness_validator.validate_effectiveness(template_content, template_name)
    ...
    def _generate_suggestions(self, issues: List[ValidationIssue], overall_score: float) -> List[str]:
        ...
        if critical_issues:
            suggestions.append("🚨 Address critical issues immediately - template may not function correctly")
        
        if error_issues:
            suggestions.append("⚠️ Fix error-level issues to improve template reliability")
        
        # Score-based suggestions
        if overall_score < 0.6:
            suggestions.append("📝 Consider major revision - template needs significant improvement")
        elif overall_score < 0.8:
            suggestions.append("✏️ Template needs moderate improvements for better quality")
        elif overall_score < 0.9:
            suggestions.append("🔧 Minor improvements would enhance template quality")
        else:
            suggestions.append("✅ Template quality is excellent - only minor optimizations needed")
        
        # Category-specific suggestions
        format_issues = [i for i in issues if i.category in ['structure', 'formatting', 'parameters']]
        content_issues = [i for i in issues if i.category in ['clarity', 'specificity', 'completeness']]
        effectiveness_issues = [i for i in issues if i.category == 'effectiveness']
        
        if len(format_issues) > 3:
            suggestions.append("📋 Focus on improving template structure and formatting")
    ...
    def validate_template_file(self, template_path: Union[str, Path]) -> TemplateValidationResult:
    ...
    def validate_all_templates(self, templates_dir: Union[str, Path]) -> Dict[str, TemplateValidationResult]:
    ...
    def generate_validation_report(self, results: Dict[str, TemplateValidationResult]) -> str:
...
def validate_template_quick(template_content: str, template_name: str = "unknown") -> Dict[str, Any]:
    ...
    return {
        ...
        'critical_issues': [i.message for i in result.issues if i.severity == ValidationSeverity.CRITICAL],
