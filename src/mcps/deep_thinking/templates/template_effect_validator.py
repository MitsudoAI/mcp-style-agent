...
class TemplateEffectValidator:
    ...
    def validate_template_effect(
        ...
        structure_score = self._test_structure_completeness(
            template_content, template_name
        )
        ...
        effectiveness_score = (
            ...
            + instruction_clarity_score
            * self.effectiveness_weights["instruction_clarity"]
            + validation_result.overall_score
            * self.effectiveness_weights["validation_score"]
        )
    ...
    def _test_instruction_following(self, content: str) -> float:
        ...
        action_verbs = len(
            re.findall(r"请|需要|要求|执行|分析|评估|生成|创建|识别|检测", content)
        )
        if action_verbs >= 5:
            score += 0.3
        elif action_verbs >= 3:
        ...
        specific_reqs = len(
            re.findall(r"必须|应该|确保|至少|不少于|不超过|\d+个|\d+分", content)
        )
        if specific_reqs >= 3:
            score += 0.3
    ...
    def _test_parameter_usage(self, content: str) -> float:
        ...
        if not parameters:
            ...
            meaningful_params = [
                p for p in param_names if len(p) > 2 and p not in ["a", "b", "c"]
            ]
            ...
            param_context = sum(
                1 for p in param_names if p in content.replace("{", "").replace("}", "")
            )
            if param_context > 0:
                score += 0.3
    ...
    def validate_all_templates(self, templates_dir: str) -> TemplateEffectReport:
        ...
        if template_metrics:
            effectiveness_scores = [
                m.effectiveness_score for m in template_metrics.values()
            ]
            average_effectiveness = sum(effectiveness_scores) / len(
                effectiveness_scores
            )
            ...
            medium_quality_count = sum(
                1 for score in effectiveness_scores if 0.6 <= score < 0.8
            )
    ...
    def _generate_global_recommendations(
        self, template_metrics: Dict[str, TemplateEffectMetrics]
    ) -> List[str]:
        ...
        low_structure_count = sum(
            1 for m in template_metrics.values() if m.structure_completeness < 0.7
        )
        low_clarity_count = sum(
            1 for m in template_metrics.values() if m.language_clarity < 0.7
        )
        low_format_count = sum(
            1 for m in template_metrics.values() if m.format_compliance < 0.7
        )
        ...
        if low_structure_count / total_templates > 0.3:
            recommendations.append(
                "Many templates lack proper structure - focus on improving section organization"
            )

        if low_clarity_count / total_templates > 0.3:
            recommendations.append(
                "Language clarity needs improvement across templates - use more specific terms"
            )

        if low_format_count / total_templates > 0.3:
            recommendations.append(
                "Format compliance issues detected - review JSON examples and heading structure"
            )

        # Quality distribution recommendations
        high_quality_ratio = (
            sum(1 for m in template_metrics.values() if m.effectiveness_score >= 0.8)
            / total_templates
        )

        if high_quality_ratio < 0.5:
            recommendations.append(
                "Less than 50% of templates meet high quality standards - consider template review"
            )
        elif high_quality_ratio > 0.8:
            recommendations.append(
                "Excellent template quality overall - maintain current standards"
            )
    ...
    def generate_detailed_report(self, report: TemplateEffectReport) -> str:
        ...
        if report.template_metrics:
            lines.extend(["## Individual Template Results", ""])

            # Sort by effectiveness score
            sorted_metrics = sorted(
                report.template_metrics.items(),
            ...
            for template_name, metrics in sorted_metrics:
                status = (
                    ...
                    else "🟡" if metrics.effectiveness_score >= 0.6 else "🔴"
                )
...
def generate_template_effect_report(
    templates_dir: str = "templates", output_file: Optional[str] = None
) -> str:
