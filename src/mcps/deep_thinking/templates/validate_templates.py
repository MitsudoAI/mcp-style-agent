...
def print_validation_summary(result: TemplateValidationResult):
    ...
    # Score bar
    score_bar = "█" * int(result.overall_score * 20) + "░" * (20 - int(result.overall_score * 20))
    ...
    if result.metrics:
        ...
        print(f"  • Critical: {result.metrics.get('critical_issues', 0)}, "
        ...
        print(f"  • Size: {result.metrics.get('word_count', 0)} words, "
              f"{result.metrics.get('line_count', 0)} lines")
        print(f"  • Scores: Format {result.metrics.get('format_score', 0):.2f}, "
...
def print_issues(result: TemplateValidationResult, show_all: bool = False):
    ...
    for severity in [ValidationSeverity.CRITICAL, ValidationSeverity.ERROR, 
        ...
        if truncated:
            print_colored(f"    ... and {len(issues_by_severity[severity]) - 3} more info issues", "blue")
...
def validate_directory(templates_dir: str, verbose: bool = False, 
    ...
    try:
        ...
        if output_format == "json":
            ...
            print(f"  • Valid Templates: {valid_templates} ({valid_templates/total_templates*100:.1f}%)")
