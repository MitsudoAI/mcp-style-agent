...
class ParameterReplacer:
        ...
        self.formatters.update(
            {
                ...
                "json_compact": lambda x: (
                    json.dumps(x, ensure_ascii=False, separators=(",", ":"))
                    if not isinstance(x, str)
                    else x
                ),
                "list": lambda x: "\n".join(
                    f"- {item}" for item in (x if isinstance(x, list) else [x])
                ),
                "numbered_list": lambda x: "\n".join(
                    f"{i+1}. {item}"
                    for i, item in enumerate(x if isinstance(x, list) else [x])
                ),
                "date": lambda x: (
                ...
                "truncate": lambda x: (
                    str(x)[:100] + "..." if len(str(x)) > 100 else str(x)
                ),
                "escape_quotes": lambda x: str(x)
                .replace('"', '\\"')
                .replace("'", "\\'"),
                "markdown_code": lambda x: f"```\n{x}\n```",
                "markdown_quote": lambda x: "\n".join(
                    f"> {line}" for line in str(x).split("\n")
                ),
            }
        )
    ...
    def _merge_variables(
        self, parameters: Dict[str, Any], context: ReplacementContext
    ) -> Dict[str, Any]:
        """Merge all available variables"""
        all_variables = {}
    ...
    def _validate_parameters(
        self, parameters: Dict[str, Any], template_params: Optional[List[str]] = None
    ):
        """Validate parameters against their configurations"""
        # Only validate parameters that are actually used in the template
        params_to_validate = (
            template_params if template_params else self.parameter_configs.keys()
        )
        ...
        for param_name in params_to_validate:
            ...
            if config.validator and not config.validator(value):
                raise ParameterValidationError(
                    f"Parameter '{param_name}' failed validation: {config.description}"
                )

    def _replace_advanced_parameters(
        ...
        def replace_match(match):
            ...
            if formatter_name and formatter_name in self.formatters:
                try:
                    ...
                    print(
                        f"Warning: Formatter '{formatter_name}' failed for parameter '{param_name}': {e}"
                    )
                ...
                if config.formatter:
                    try:
                        ...
                        print(
                            f"Warning: Parameter formatter failed for '{param_name}': {e}"
                        )
    ...
    def _replace_simple_parameters(
        self, template: str, variables: Dict[str, Any]
    ) -> str:
        ...
        for param in parameters:
            if param in self.parameter_configs:
                ...
                analysis["suggestions"].append(
                    f"Consider registering configuration for parameter '{param}'"
                )

        # Check for potential issues
        if not parameters:
            analysis["suggestions"].append(
                "Template contains no parameters - consider if this is intentional"
            )

        if len(analysis["unknown_parameters"]) > 0:
            analysis["validation_errors"].append(
                f"Unknown parameters: {', '.join(analysis['unknown_parameters'])}"
            )
